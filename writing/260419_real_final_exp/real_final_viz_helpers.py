...
SHARED_DATASET_METRIC_LIMITS = {
    "KuaiRecLargeStrictPosV2_0.2": {
        "ndcg20": (0.2666, 0.3538),
        "hr10": (0.3061, 0.3669),
    },
    "foursquare": {
        "ndcg20": (0.1580, 0.2184),
        "hr10": (0.2180, 0.3215),
    },
}
...
def shared_dataset_metric_limits(
    ...
    # 데이터가 CANONICAL 범위를 크게 벗어나는 경우(특히 outlier)에만 범위를 확장합니다.
    # 단, outlier가 아래로 너무 처진 경우 전체 스케일이 깨질 수 있으므로
    # canon_min보다 작을 때만 확장하도록 하여 '낮은 것'을 대응합니다.
...
def bar_line_panel(
    ...
    ax.set_ylim(*shared_dataset_metric_limits(work, ndcg_col, ndcg_vals))
    twin.set_ylim(*shared_dataset_metric_limits(work, hr_col, hr_vals))
    ax.margins(x=0.06)
    clean_axes(ax)
    twin.grid(False)
