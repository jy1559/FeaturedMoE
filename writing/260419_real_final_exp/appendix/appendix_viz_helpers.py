...
def bar_line_panel(
    ...
    twin = ax.twinx()
    twin.plot(x, line_vals, color=PALETTE["ink"], marker="o",
              linewidth=2.1, markersize=5.4, zorder=3)
    ...
    ax.set_ylim(*metric_limits(bar_vals, padding=0.18, floor=0.0))
    twin.set_ylim(*metric_limits(line_vals, padding=0.14, floor=0.0))
    ax.margins(x=0.06)
    clean_axes(ax)
    twin.grid(False)
