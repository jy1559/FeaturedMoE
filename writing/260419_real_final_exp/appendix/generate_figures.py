...
remove_panel_label_space = viz.remove_panel_label_space
PALETTE        = viz.PALETTE
...
def fig_a02_structural() -> None:
    ...
    for group_key, group_name, fig_name, legacy_name in [
        ...
        fig, ax = plt.subplots(figsize=(7.55, 4.45), constrained_layout=True)
        bar_line_panel(
            avg,
...
def fig_a03_sparse_and_objective() -> None:
    ...
    if not diag_df.empty:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11.6, 4.25), constrained_layout=True)
    ...
    if not route_diag.empty:
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11.2, 4.35), constrained_layout=True)
        ...
        if not heat_ent.empty:
            sns.heatmap(heat_ent, ax=ax1, cmap="YlOrRd", annot=True, fmt=".2f",
                        annot_kws={"size": 10}, linewidths=0.5, cbar_kws={"shrink": 0.86})
...
def fig_a04_bins_and_slices() -> None:
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11.6, 4.1), constrained_layout=True)
    ...
    fig, axes = plt.subplots(1, len(datasets_i), figsize=(4.25 * len(datasets_i), 4.1),
    ...
    for ax, ds in zip(axes, datasets_i):
        ...
        bar_line_panel(sub_rr, category_col="slice_label",
                       ...
                       xrotation=0)
        ax.set_title(DATASET_LABELS.get(ds, ds), fontsize=11.4)
    ...
    if not sasrec_s.isna().all() and not route_s.isna().all():
        ...
        ax.set_ylim(*metric_limits(gain_s.values, padding=0.24))
        clean_axes(ax)
...
def fig_a05_interventions() -> None:
    ...
    # ── Fig J1(a): Score drop ──
    INV_ORDER = ["feature_zero_memory", "feature_zero_focus", "feature_zero_tempo", "feature_zero_exposure"]
    INV_LABELS = {
        "feature_zero_memory": "Zero Memory",
        "feature_zero_focus": "Zero Focus",
        "feature_zero_tempo": "Zero Tempo",
        "feature_zero_exposure": "Zero Exposure",
    }
    PAL_LIST = [PALETTE["blue"], PALETTE["orange"], PALETTE["route"], PALETTE["plum"]]
    ...
    if drops:
        ax.set_ylim(*metric_limits(drops, padding=0.24))
    clean_axes(ax)
    savefig("a05_intervention_score_drop.pdf", fig)

    # ── Fig J1(b): Stage routing profiles ──
    CASE_GROUPS = ["repeat_heavy", "fast_exploratory", "narrow_focus", "preference_stable"]
    STAGE_ORDER = ["macro", "mid", "micro"]
    ROUTED_FAMS = ["memory", "focus", "tempo", "exposure"]
    CASE_LABELS = {
        "repeat_heavy": "Repeat-heavy",
        "fast_exploratory": "Fast exploratory",
        "narrow_focus": "Narrow-focus",
        "preference_stable": "Preference-stable",
    }
    ...
    fig, axes = plt.subplots(1, len(case_groups_present),
                              figsize=(3.85 * len(case_groups_present), 4.05),
    ...
    for ax, grp in zip(axes, case_groups_present):
        ...
        hm = (sub.groupby(["routed_family", "stage_name"])["usage_share"].mean()
              ...
              .fillna(0))
        sns.heatmap(hm, ax=ax, cmap="crest", vmin=0, vmax=max(0.45, float(hm.to_numpy().max())), linewidths=0.5,
                    annot=True, fmt=".2f", annot_kws={"size": 8.8},
                    cbar=(ax is axes[-1]))
        ax.set_title(CASE_LABELS.get(grp, grp), fontsize=10.6)
        ax.set_xlabel("Stage")
...
def fig_a06_transfer() -> None:
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12.4, 4.15), constrained_layout=True)
    ...
    ax1.legend(fontsize=8.8, ncol=2, loc="upper left")
    clean_axes(ax1)
    ...
    ax2.legend(fontsize=8.8, ncol=2, loc="upper left")
    clean_axes(ax2)
