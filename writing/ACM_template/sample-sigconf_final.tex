...
\begin{abstract}
In sessionized sequential recommendation, sessions are short but cross-session history is available. This brevity sharpens behavioral differences across sessions: some sessions are repeat-heavy, others fast and exploratory, others narrowly focused. Existing models use behavioral context to enrich the sequence representation — improving \emph{what the model encodes}. We ask whether behavioral context should instead determine \emph{which computation path} the model applies, and what observable signal should drive that routing decision.

\textbf{RouteRec} pursues this direction with Mixture of Experts (MoE). The central design choice is the routing input: hidden states are optimized for next-item prediction rather than for capturing the behavioral character of a session, and routing from them places routing control and representation on the same vector — the routing input is identical to the quantity that routing itself shapes. RouteRec instead computes \emph{behavioral cues} — compact statistics derived directly from interaction logs — and routes on them through a control path kept strictly separate from the sequential backbone. This separation makes the routing input explicit, grounds route choices in observable quantities, and keeps representation and routing control from interfering. RouteRec extracts cues at macro, mid, and micro temporal scopes and applies hierarchical sparse expert allocation to concentrate computation on a small, focused expert subset.
...
\label{sec:intro}
Sequential recommendation assigns the same shared computation to every user sequence. In session-based settings — where interactions are partitioned into short, temporally bounded sessions — behavioral differences across sessions become particularly pronounced: a session built on rapid revisits to familiar items has a structurally different character from one sweeping across diverse categories in quick succession, even when both users share similar long-run preferences. Pace, repetition patterns, category focus, and popularity exposure are directly observable from interaction logs and vary consistently across users and time \cite{jannach2022sessions,wang2021sessionsurvey}.
...
Figure~\ref{fig:intro-behavioral-features} illustrates the immediate consequence: sessions with visibly different structural character still pass through the same computation. Existing work addresses behavioral differences by enriching the sequence representation — time-aware self-attention \cite{li2020tisasrec} embeds interval structure into the backbone, cross-session architectures \cite{quadrana2017hrnn} bring in longer preference context, and feature-aware models \cite{zhang2019fdsa,xie2022difsr} incorporate side information into the encoding path. These approaches improve \emph{what the model represents}. They still apply the same transformation to every prefix, regardless of structural differences across sessions. Whether behavioral context should also determine \emph{which transformation the model applies} — rather than only enriching what a single shared block processes — is a question they do not address.

Conditional computation — executing only a data-dependent subset of model parameters, rather than the full model for every input — makes this question concrete. Mixture of Experts (MoE) \cite{jacobs1991adaptive,jordan1994hme} provides the mechanism: a router that selects which subset of expert modules (each a small feed-forward network) processes each input, rather than passing all inputs through the same shared transformation. Sparse MoE \cite{shazeer2017moe,fedus2022switch,du2022glam,zoph2022stmoe} makes that selection efficient by activating only a small subset at a time. Once MoE is introduced, the routing input becomes a substantive design choice. The natural default is to route from hidden states, since they summarize the sequence. But hidden states are optimized for next-item prediction, not for capturing the behavioral regime of a session — its observable structural character, such as repeat-heavy consumption or fast exploratory browsing. Routing from hidden states also entangles two roles in a single vector: the quantity driving expert selection is identical to the quantity being shaped by that selection, so the behavioral basis of any given route stays implicit. In short sessions, a hidden state computed from a few interactions does not yet capture cross-session behavioral patterns at the point when routing decisions are made.

RouteRec keeps two channels distinct: a \emph{sequential backbone} that encodes the sequence for next-item prediction, and a dedicated \emph{routing control path} that computes \emph{behavioral cues} — compact scalar summaries derived directly from interaction logs — and uses them to select experts. Behavioral cues are organized into four cue families that cover the main observable dimensions of session structure: Tempo (interaction pace and interval dynamics), Focus (category concentration and switching), Memory (repetition and recurrence), and Exposure (popularity level and drift). Cues are computed at three temporal scopes — macro (cross-session history), mid (within-session dynamics), and micro (short-term local transitions) — because each exposes distinct behavioral evidence. Because cues are observable statistics rather than learned latent values, route choices can be traced back to specific behavioral quantities visible in the log.

Three tensions arise in instantiating this design. Behavioral cues must remain portable across datasets where only item identities, timestamps, and sometimes coarse category labels are available, yet carry enough signal to differentiate computation paths — the specific expert modules activated for a given prefix, or its \emph{route}. Routing decisions must be separated by temporal scope so that slow cross-session tendency and fast local dynamics each receive a distinct control view. Expert selection must enforce \emph{route commitment}: routing that concentrates on a small, consistent expert subset rather than diffusing weight broadly, so that behaviorally different sessions activate genuinely different expert subsets. Section~\ref{sec:problem} formalizes the task and states each tension precisely.

The main contributions of this work are:
\begin{itemize}
  \item We identify the routing input as a substantive design decision in sessionized sequential recommendation and argue that behavioral cues computed directly from interaction logs — kept on a control path separate from the sequential backbone — provide a cleaner routing interface than hidden states, avoiding the entanglement that arises when the routing input and the quantity it shapes occupy the same vector.
  \item We propose RouteRec, which instantiates this design through three components: Behavioral Cue Construction, Coarse-to-Fine Multi-Stage Routing, and Hierarchical Sparse Expert Allocation.
...
Work on sparse MoE mechanics, including expert choice routing \cite{zhou2022expertchoice} and z-loss stabilization \cite{zoph2022stmoe}, improves how sparse activation is executed. It does not determine what the router should read. That question is especially consequential in session-based sequential recommendation: routing decisions are made from short prefixes whose hidden states do not yet expose cross-session behavioral character, so the behavioral basis of any given route stays implicit regardless of the routing mechanics. RouteRec targets this gap by defining behavioral cues directly from interaction logs and keeping the routing control path separate from the representation path.
...
Using behavioral cues from interaction logs as the explicit routing input — rather than hidden states — raises three design tensions. Each must be resolved for cue-driven routing to produce systematic computation differences across sessions, rather than adding a routing step that reduces to a near-uniform mixture regardless of the cue.
...
Figure~\ref{fig:framework} shows RouteRec at the framework level: panel (a) covers cue extraction, panel (b) the sequential backbone layout, and panel (c) hierarchical sparse expert selection. The model follows the two-path design from Section~\ref{sec:problem}: the sequential backbone encodes the sequence for next-item prediction, while a separate routing control path computes behavioral cues from interaction logs and uses them to select experts at each stage. Three components instantiate this design — Behavioral Cue Construction addresses the portability–informativeness tension (C1), Coarse-to-Fine Multi-Stage Routing separates routing decisions by temporal scope (C2), and Hierarchical Sparse Expert Allocation enforces route commitment (C3). Alternative layout variants and broader structural sweeps are in Appendix~\ref{app:method-details}.

\subsection{Behavioral Cue Construction}
\label{sec:feature-construction}

The routing control path must operate under the data conditions present in most benchmark settings, where the available signals after sessionization are narrow: item identity, timestamp, optional coarse grouping label (category, genre, artist, venue type), and corpus-level popularity statistics computed from training data. RouteRec derives behavioral cues exclusively from these signals, without requiring side-information encoders or auxiliary supervision in the routing branch.

For session $s_m$ and target position $t$, the three temporal windows are
\begin{align}
...
\end{align}
where $H=5$ past sessions and $w=5$ positions in the main configuration; Appendix~\ref{app:stage-layout} reports ablations on these window sizes. Let $\mathcal{T}=\{\mathrm{macro},\mathrm{mid},\mathrm{micro}\}$ denote the routing-stage set. A shared summary operator $\mathcal{A}(\cdot)$ maps each window to a cue vector:
\[
\mathbf{f}^{(s)}_t=\mathcal{A}(\mathcal{R}^{(s)}_t),\qquad s\in\mathcal{T}.
\]

The cue vector is organized into the four cue families — Tempo, Focus, Memory, and Exposure — introduced in Section~\ref{sec:intro}. In the main configuration, this yields a 16-dimensional cue vector per stage. When the coarse grouping field is absent, Focus cues default to the fixed missing-signal value documented in Appendix~\ref{app:method-details}. Representative cue definitions appear in Table~\ref{tab:appendix-feature-families}.

The same template applies at all three scopes, but each window exposes different behavioral evidence: macro reflects persistent cross-session tendency, mid tracks evolving within-session dynamics, and micro captures short-horizon transitions near the prediction target. This consistency across scopes means a single cue bank serves as a portable routing interface without requiring dataset-specific feature engineering.

\subsection{Coarse-to-Fine Multi-Stage Routing}
\label{sec:multi-stage-routing}

Routing decisions made at different temporal scopes require different behavioral evidence. RouteRec addresses this by placing three routing stages at distinct positions along the backbone, each drawing on its corresponding window. In the main configuration, the serial layout is
\[
\underbrace{\text{self-attn}}_{\text{context}} \to \text{macro} \to \text{mid} \to \underbrace{\text{self-attn}}_{\text{refresh}} \to \text{micro}.
\]
Let $\mathbf{h}^{(0)}_t$ denote the hidden state after the first attention block, $\mathbf{h}^{(1)}_t$ after macro routing, $\mathbf{h}^{(2)}_t$ after mid routing, and $\mathbf{h}^{(3)}_t$ after the second attention block. Macro, mid, and micro consume $\mathbf{h}^{(0)}_t$, $\mathbf{h}^{(1)}_t$, and $\mathbf{h}^{(3)}_t$ as their respective inputs $\mathbf{h}^{\mathrm{in},(s)}_t$. The first attention block provides a contextualized sequence state before the slower macro and mid decisions; the second refreshes token interactions immediately before the local micro stage. Appendix~\ref{app:stage-layout} reports ablations on alternative layout orderings.

\paragraph{Macro and mid (sequence-wise routing).}
Macro and mid each produce one routing distribution per sequence from their respective windows and broadcast it over all valid positions. This treats behavioral tendency at these scopes as a sequence-level signal: position-wise routing at macro and mid added variance without improving control resolution in our ablations. Let $\bar{\mathbf{f}}^{(s)}_i = \tfrac{1}{L_i}\sum_{t=1}^{L_i}\mathbf{f}^{(s)}_{i,t}$ be the sequence-level cue summary for stage $s \in \{\mathrm{macro},\mathrm{mid}\}$, where $L_i$ is the valid sequence length. The routing distribution is computed once per sequence and broadcast to all positions:
\begin{align}
\bar{\boldsymbol{\pi}}^{(s)}_i
&=
\mathrm{softmax}\!\left(\mathrm{Router}_s(\bar{\mathbf{f}}^{(s)}_i)/\tau_s\right), \\
...
Micro computes a separate routing distribution at each position using the short-horizon window $\mathcal{R}^{\mathrm{micro}}_t$, allowing the route to respond to immediate transition patterns:
\[
...
The coarse-to-fine character of RouteRec comes from evidence scope and update frequency: macro and mid commit to one route per sequence, micro adapts per position. Each stage captures a distinct behavioral control view. Section~\ref{sec:hierarchical-routing} specifies how expert selection within each stage enforces route commitment.

\subsection{Hierarchical Sparse Expert Allocation}
\label{sec:hierarchical-routing}

Even with explicit cue-driven routing, behavior-dependent computation collapses if routing spreads weight across all experts — a repeat-heavy session and a fast-exploratory one would then activate nearly identical mixtures regardless of the cue. RouteRec enforces route commitment through a hierarchical factored router that makes a group-level decision before a within-group decision. Experts are partitioned into $G$ groups with $C$ experts per group, so $E=GC$ experts in total. In the main configuration, groups are aligned with the four cue families, making the group-level choice correspond to a cue family; multiple experts within each group expand capacity within that family. All three stages share the same cue-driven group-conditional router structure.

For stage $s$ and position $t$, let $z^{(s)}_{t,g}$ and $u^{(s)}_{t,g,c}$ be learned linear projections of the cue vector $\mathbf{f}^{(s)}_t$. The router produces a group-level distribution and a conditional within-group distribution:
\begin{align}
p^{(s)}_{t,g} &= \mathrm{softmax}_g\!\left(z^{(s)}_{t,g}\right), \\
r^{(s)}_{t,g,c} &= \mathrm{softmax}_c\!\left(u^{(s)}_{t,g,c}\right).
\end{align}
Their product gives the joint expert weight:
\begin{equation}
...
Route commitment requires that only a small subset of groups and experts executes at each decision. Let $\operatorname{TopK}(\mathbf{v},k)$ denote the index set of the $k$ largest entries of $\mathbf{v}$. We retain the top-$k_g$ groups:
\begin{equation}
\bar{p}^{(s)}_{t,g}
=
...
\tilde{\mathbf{h}}^{(s)}_t
=
\sum_{g,c}\hat{\pi}^{(s)}_{t,g,c}\,\mathrm{Expert}^{(s)}_{g,c}(\mathbf{h}^{\mathrm{in},(s)}_t).
\label{eq:sparse-stage-routing}
\end{equation}
In the main configuration, $G=4$ and $C=3$, with $k_g=3$ and $k_e=2$, so each routing decision activates $6$ of $12$ experts. The group-level prior remains soft at the score level; the executed computation path is explicitly sparse.

Sparsity here is not primarily a computational optimization. A dense soft mixture reduces route differences across sessions because every expert contributes to every output, regardless of the cue. Sparsity enforces the commitment that makes routing consequential: sessions with different behavioral character activate different expert subsets. The hierarchical factorization keeps that selection inspectable — the group-level choice corresponds to a cue family, and the within-group choice to a specific capacity expansion — while the experts themselves remain fully learned. Dense soft mixing is evaluated as an ablation in Q3.

\subsection{Training Objective}
\label{sec:training-objective}

RouteRec is trained with a standard next-item objective and two router-side regularizers in the main configuration: route consistency and z-loss.
...
\]
We also form a stage-level cue summary
\[
\bar{\mathbf{f}}^{(s)}_i
=
\frac{1}{L_i}\sum_{t=1}^{L_i}\mathbf{f}^{(s)}_{i,t}.
\]
...
To prevent the pre-softmax routing logits from growing unbounded, we adopt z-loss on the pre-softmax expert logits $a_{i,t,g,c}^{\,s}$~\cite{zoph2022stmoe}:
\[
\mathcal{L}_{z}
=
\frac{1}{|\mathcal{T}|}
...
\begin{figure}[!t]
  ...
  \caption{Routing-control ablations on KuaiRec and Foursquare. The five variants compare a shared FFN baseline, hidden-state-only routing, mixed hidden+behavior routing, additive fusion bias, and the final behavior-guided router. Across both datasets, direct behavioral control yields the most consistent performance.}
...
\begin{figure}[!t]
  ...
  \caption{Design-justification ablations on KuaiRec. Panel (a) compares three-scope, two-scope, and one-scope variants to test whether separating temporal scopes is necessary. Panel (b) compares hierarchical versus flat routing and sparse versus dense allocation under comparable expert budgets.}
  \Description{One-column figure with two half-width legends above two subfigures. Both Q3 panels are measured on KuaiRec, covering temporal decomposition and routing organization.}
...
\begin{figure}[!t]
  ...
  \caption{Reduced-cue portability under retraining. Each panel compares the full model against reduced-cue variants to show how much performance remains when time or group cues are removed.}
  \Description{One-column figure with a full-width portability legend above two subfigures for KuaiRec and Foursquare.}
  \label{fig:q4-portability-panels}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{Fixed-model cue efficacy under evaluation-time perturbation. Each panel applies inference-time cue perturbations to the trained model to test how strongly performance depends on intact behavioral features.}
  \Description{One-column figure with a full-width cue-efficacy legend above two subfigures for KuaiRec and Foursquare.}
  \label{fig:q4-efficacy-panels}
\end{figure}
