...
BAL_SPACE="${BAL_SPACE_OVERRIDE:-0.0025,0.0032,0.0042}"
...
[ -z "$EXP_NAME" ] && EXP_NAME="fmoe_hgr_v3_${PHASE%%_*}_hparam"
[ -z "$EXP_DESC" ] && EXP_DESC="HGRv3 quick hparam search around fixed layout/anchor with inner rule mode=${INNER_RULE_MODE}."
[ -z "$EXP_FOCUS" ] && EXP_FOCUS="arch_layout_id,embedding_size,d_expert_hidden,d_router_hidden,expert_scale,inner_rule_mode,inner_rule_lambda,inner_rule_bias_scale,learning_rate,weight_decay,hidden_dropout_prob,balance_loss_lambda"

LOG_FILE_PATH="$(run_make_phase_log_path fmoe_hgr_v3 hparam "$DATASET" "FeaturedMoE_HGRv3" "$PHASE")"
PHASE_SUMMARY_DIR="$(run_log_dir fmoe_hgr_v3)/hparam/${PHASE_BUCKET}"
...
cmd=(
  ...
  --seed "$SEED"
  --run-group fmoe_hgr_v3
  --run-axis hparam
  --run-phase "$PHASE"
  "model=featured_moe_hgr_v3_tune"
  "dataset=${DATASET}"
  "eval_mode=session"
  ...
  "++search.group_router_mode=[stage_wide]"
...
core_setup:
  layout=${LAYOUT_ID} | merge=serial | outer=stage_wide(hidden=${OUTER_ROUTER_USE_HIDDEN},feature=${OUTER_ROUTER_USE_FEATURE}) | inner=hidden+group_feature
...
if ! "$PY_BIN" "${RUN_DIR}/common/phase_experiment_summary.py" \
  --results-dir "$(run_results_dir fmoe_hgr_v3)" \
  --dataset "$DATASET" \
  --phase-bucket "$PHASE_BUCKET" \
  --output-csv "$PHASE_SUMMARY_CSV" \
  --output-md "$PHASE_SUMMARY_MD" \
  --title "HGRv3 ${PHASE_BUCKET} Summary" \
  --notes "hidden-only outer, weak inner distill probe, compare expert_top_k under fixed layout 15"; then
