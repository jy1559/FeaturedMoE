...
Usage: $0 [--datasets KuaiRecLargeStrictPosV2_0.2] [--gpus 4,5,6,7]
          [--hparams 1] [--seeds 1]
          [--max-evals 10] [--tune-epochs 100] [--tune-patience 10] [--seed-base 72000]
          [--feature-group-bias-lambda 0.05] [--rule-bias-scale 0.1]
          [--z-loss-lambda 1e-4] [--balance-loss-lambda 0.0] [--macro-history-window 5]
          [--only-setting P12-00_ATTN_ONESHOT,P12-25_BUNDLE_ALL_ROUTER_CONDITIONED]
...
for DATASET in "${DATASET_LIST[@]}"; do
  ...
  CMD=(
    "${PYTHON_BIN}"
    "${SCRIPT_DIR}/run_phase12_layout_composition.py"
    --dataset "${DATASET}"
    --gpus "${GPU_LIST}"
    --hparams "${HPARAMS}"
    --seeds "${SEEDS}"
...
echo "[All Done] phase12 layout composition datasets completed in order: ${DATASETS}"
