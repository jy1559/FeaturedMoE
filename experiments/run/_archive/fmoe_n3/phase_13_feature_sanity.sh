...
Usage: $0 [--datasets KuaiRecLargeStrictPosV2_0.2] [--gpus 4,5,6,7]
          [--hparams 1] [--seeds 1]
          [--max-evals 10] [--tune-epochs 100] [--tune-patience 10] [--seed-base 73000]
          [--feature-group-bias-lambda 0.05] [--rule-bias-scale 0.1]
          [--z-loss-lambda 1e-4] [--balance-loss-lambda 0.0] [--macro-history-window 5]
          [--only-setting P13-00_FULL_DATA,P13-23_TRAIN_POSITION_SHIFT_PLUS3]
...
for DATASET in "${DATASET_LIST[@]}"; do
  ...
  CMD=(
    "${PYTHON_BIN}"
    "${SCRIPT_DIR}/run_phase13_feature_sanity.py"
    --dataset "${DATASET}"
    --gpus "${GPU_LIST}"
    --hparams "${HPARAMS}"
    --seeds "${SEEDS}"
...
echo "[All Done] phase13 feature sanity datasets completed in order: ${DATASETS}"
