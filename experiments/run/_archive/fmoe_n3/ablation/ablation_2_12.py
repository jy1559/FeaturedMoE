#!/usr/bin/env python3
"""Launch KuaiRec-only A12 ablation v2 (12 settings) for FMoE_N3.

Matrix design:
- Feature-group subset ablations (6): 1-group x2, 2-group x2, 3-group x2
- Aux-loss ablations (6): balance-only, none, knn-only, knn+balance, plus 2 lambda tweaks
"""
...
AXIS = "ablation_feature_add_v3_a12_kuairec_v2"
PHASE_ID = "P17B"
PHASE_NAME = "A12_ABLATION2_12"
RUN_STAGE = "wide"
AXIS_ID = "B"
AXIS_DESC = "kuairec_a12_ablation2_12"
...
def _manifest_path(args: argparse.Namespace) -> Path:
    ...
    return _phase_log_dir(args.dataset) / "ablation2_12_matrix.json"


def _stage_mask(selected_groups: list[str]) -> Dict[str, list[str]]:
    groups = [g for g in FEATURE_GROUPS if g in set(selected_groups)]
    return {
        "macro": list(groups),
        "mid": list(groups),
        "micro": list(groups),
...
def _setting(
    *,
    ...
    return {
        ...
        "setting_key": f"ABL2-{int(idx):02d}_{key}",
        "setting_desc": key,
        "setting_group": group,
...
def _build_settings(args: argparse.Namespace) -> list[Dict[str, Any]]:
    ...
    # 1) Feature-group subset ablations (6)
    settings.append(
        _setting(
            idx=1,
            key="FG_ONLY_MEMORY",
            group="feature_group_subset",
            detail="Use only Memory feature group",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Memory"])},
        )
    )
    settings.append(
        _setting(
            idx=2,
            key="FG_ONLY_EXPOSURE",
            group="feature_group_subset",
            detail="Use only Exposure feature group",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Exposure"])},
        )
    )
    settings.append(
        _setting(
            idx=3,
            key="FG_MEMORY_EXPOSURE",
            group="feature_group_subset",
            detail="Use Memory + Exposure only",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Memory", "Exposure"])},
        )
    )
    settings.append(
        _setting(
            ...
            key="FG_FOCUS_MEMORY",
            group="feature_group_subset",
            detail="Use Focus + Memory only",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Focus", "Memory"])},
        )
    )
    settings.append(
        _setting(
            ...
            key="FG_NO_TEMPO",
            group="feature_group_subset",
            detail="Use 3 groups (drop Tempo)",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Focus", "Memory", "Exposure"])},
        )
    )
    settings.append(
        _setting(
            ...
            key="FG_NO_EXPOSURE",
            group="feature_group_subset",
            detail="Use 3 groups (drop Exposure)",
            base=base,
            extra_overrides={"stage_feature_family_mask": _stage_mask(["Tempo", "Focus", "Memory"])},
        )
    )

    # 2) Aux-loss ablations (6)
    settings.append(
        _setting(
            idx=7,
            key="AUX_BALANCE_ONLY",
            group="aux_loss",
            detail="balance only (no knn, no z-loss)",
            base=base,
            extra_overrides={
                "route_consistency_lambda": 0.0,
                "z_loss_lambda": 0.0,
                "balance_loss_lambda": float(args.aux_balance_lambda),
            },
        )
    )
    settings.append(
    ...
    settings.append(
        _setting(
            ...
            key="AUX_KNN_ONLY",
            group="aux_loss",
            detail="knn alignment only",
            base=base,
            extra_overrides={
                "route_consistency_lambda": float(args.a2_route_consistency_lambda),
                "z_loss_lambda": 0.0,
                "balance_loss_lambda": 0.0,
            },
        )
    ...
    settings.append(
        _setting(
            idx=11,
            key="AUX_KNN_ONLY_LAMBDA_UP",
            group="aux_loss",
            detail="knn only with stronger lambda",
            base=base,
            extra_overrides={
                "route_consistency_lambda": float(args.knn_lambda_high),
                "z_loss_lambda": 0.0,
                "balance_loss_lambda": 0.0,
            },
        )
    ...
    settings.append(
        _setting(
            idx=12,
            key="AUX_KNN_BALANCE_LAMBDA_UP",
            group="aux_loss",
            detail="knn + balance with stronger lambdas",
            base=base,
            extra_overrides={
                "route_consistency_lambda": float(args.knn_lambda_high),
                "z_loss_lambda": 0.0,
                "balance_loss_lambda": float(args.aux_balance_lambda_high),
            },
        )
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FMoE_N3 KuaiRec A12 ablation2-12 launcher")
    ...
    parser.add_argument("--aux-balance-lambda", type=float, default=2e-4)
    parser.add_argument("--knn-lambda-high", type=float, default=1.2e-3)
    parser.add_argument("--aux-balance-lambda-high", type=float, default=4e-4)
...
def _apply_smoke_mode(args: argparse.Namespace) -> None:
    ...
    if not args.only_setting:
        args.only_setting = "ABL2-01_FG_ONLY_MEMORY,ABL2-07_AUX_BALANCE_ONLY"
