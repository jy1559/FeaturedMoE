...
Usage: $0 --dataset <ds> [--layout-id N] [--execution serial|parallel]
          ...
          [--search-profile wide|teacher|router|bias] [--gpu N]
...
case "$SEARCH_PROFILE" in
  wide)
    LR_SPACE='[2e-4,3e-4,4.5e-4,6e-4,8e-4,1.1e-3,1.5e-3,2.0e-3]'
    WD_SPACE='[5e-5]'
    DROP_SPACE='[0.10]'
    BAL_SPACE='[0.005]'
    ;;
  teacher)
    ...
    ;;
  router)
    LR_SPACE='[6e-4,9e-4,1.2e-3,1.6e-3,2.1e-3,2.8e-3,3.6e-3,4.8e-3]'
    WD_SPACE='[5e-5]'
    DROP_SPACE='[0.10]'
    BAL_SPACE='[0.005]'
    ;;
  bias)
    LR_SPACE='[1.5e-4,2.5e-4,3.5e-4,5e-4,7e-4,9e-4,1.2e-3,1.6e-3]'
    WD_SPACE='[5e-5]'
    DROP_SPACE='[0.10]'
    BAL_SPACE='[0.005]'
    ;;
...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v4d_${PHASE%%_*}_hparam"
fi
if [ -z "$EXP_DESC" ]; then
  EXP_DESC="FeaturedMoE_v4_Distillation fixed-anchor hyperopt: flat router + direct 12-way teacher/fusion search."
fi
if [ -z "$EXP_FOCUS" ]; then
  EXP_FOCUS="router_design,teacher_design,teacher_delivery,teacher_stage_mask,teacher_kl_lambda,teacher_bias_scale,teacher_until,learning_rate"
fi
...
LOG_FILE_PATH="$(run_make_sequential_phase_log_path fmoe_v4_distillation hparam "$DATASET" "FeaturedMoE_v4_Distillation_${EXECUTION}" "$PHASE")"
...
cmd=(
  ...
  --seed "$SEED"
  --run-group fmoe_v4_distillation
  --run-axis hparam
  --run-phase "$PHASE"
  ...
  "++search.router_design=[flat_legacy]"
  "router_clone_residual_scale=0.20"
  "++search.router_clone_residual_scale=[0.20]"
  "teacher_design=none"
  "++search.teacher_design=[none]"
  "teacher_delivery=none"
  "++search.teacher_delivery=[none]"
  "teacher_stage_mask=all"
  "++search.teacher_stage_mask=[all]"
  "teacher_kl_lambda=0.0"
  "++search.teacher_kl_lambda=[0.0]"
  "teacher_bias_scale=0.0"
  "++search.teacher_bias_scale=[0.0]"
  "teacher_temperature=1.5"
  "++search.teacher_temperature=[1.5]"
  "teacher_until=0.25"
  "++search.teacher_until=[0.25]"
  "teacher_stat_sharpness=16.0"
  "++search.teacher_stat_sharpness=[16.0]"
  "router_impl=learned"
...
RUN_ID="$(run_tracker_start \
  ...
  --phase "$PHASE" \
  --dataset "$DATASET" \
  --model "FeaturedMoE_v4_Distillation_${EXECUTION}" \
  --exp-name "$EXP_NAME" \
  --exp-desc "$EXP_DESC" \
  --exp-focus "$EXP_FOCUS" \
  --cmd "$CMD_STR" \
...
run_update_model_report \
  ...
  "$(run_experiments_dir)/models/FeaturedMoE_v4_Distillation"
run_update_track_report fmoe_v4_distillation
