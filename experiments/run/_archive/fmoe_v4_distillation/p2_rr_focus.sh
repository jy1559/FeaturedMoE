...
latest_phase_log() {
  ...
  dir="$(run_log_dir fmoe_v3)/hparam/${phase_bucket}/${dataset_tag}/${model_tag}"
  [ -d "$dir" ] || { echo ""; return 0; }
...
find_best_parent_result() {
  local dataset="$1"
  local layout_id="$2"
  python3 - <<'PY' "$(run_results_dir fmoe_v3)" "$dataset" "$layout_id"
import glob
import json
import os
import sys
...
pattern = os.path.join(base, f"{dataset}_FeaturedMoE_v3_p1s_*_l{layout}_*.json")
best_path = ""
best_mrr = None
