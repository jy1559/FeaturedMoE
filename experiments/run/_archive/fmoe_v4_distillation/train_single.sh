...
Usage: $0 --dataset <movielens1m|retail_rocket> [--gpu N] [--seed N]
          [--layout-id N] [--execution serial|parallel] [--schedule off|on]
...
load_defaults() {
  case "$DATASET" in
    movielens1m)
      MAX_LEN="10"; EMB="128"; D_FEAT="16"; D_EXP="256"; D_ROUT="64"; SCALE="3"; HEADS="8"
      LR="0.0012"; WD="5e-5"; DROP="0.10"; BAL="0.005"
      TRAIN_BS="4096"; EVAL_BS="8192";;
    retail_rocket)
      MAX_LEN="10"; EMB="128"; D_FEAT="16"; D_EXP="160"; D_ROUT="64"; SCALE="3"; HEADS="8"
      LR="5e-4"; WD="5e-5"; DROP="0.10"; BAL="0.005"
      TRAIN_BS="4096"; EVAL_BS="8192";;
    *)
...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v4d_${PHASE%%_*}_train_${EXECUTION}"
fi
if [ -z "$EXP_DESC" ]; then
  EXP_DESC="FeaturedMoE_v4_Distillation single run for anchor/smoke verification."
fi
if [ -z "$EXP_FOCUS" ]; then
  EXP_FOCUS="router_design,teacher_design,teacher_delivery,teacher_stage_mask,learning_rate,train_batch_size,eval_batch_size"
fi
...
LOG_FILE_PATH="$(run_make_log_path fmoe_v4_distillation train "$DATASET" "FeaturedMoE_v4_Distillation_${EXECUTION}" "$GPU_ID" "$PHASE")"

cmd=(
  "$PY_BIN" recbole_train.py
  "model=featured_moe_v4_distillation"
  "dataset=${DATASET}"
  "eval_mode=session"
