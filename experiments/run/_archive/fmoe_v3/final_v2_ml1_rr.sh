...
topn = int(sys.argv[3])
root = Path(os.environ.get("HYPEROPT_RESULTS_DIR", "run/artifacts/results")) / "fmoe_v3"
if not root.exists():
    raise SystemExit(0)

rows = []
for p in root.glob(f"{dataset}_FeaturedMoE_v3_*.json"):
    try:
  ...
  for gidx in "${!GPUS[@]}"; do
    ...
    (
      ...
      for slot in $(seq 0 $((COMBOS_PER_GPU - 1))); do
        ...
        phase="FINALV3_ML1_G${gpu}_C$((slot + 1))_${arm_tag}_${profile_tag}_R${repeat_idx}"
...
if [ "$DRY_RUN" != "true" ]; then
  mapfile -t transfer_parents < <(phase_top_results movielens1m FINALV3_ML1_ 2)
fi
...
if [ "${#transfer_parents[@]}" -eq 0 ]; then
  ...
  run_v2_tune retail_rocket "${GPUS[0]}" FINALV3_RR_T1 "${RR_TRANSFER_EVALS}" "$((SEED_BASE + 21))" "" "${transfer_overrides_1[@]}"
  run_v2_tune retail_rocket "${GPUS[$((1 % ${#GPUS[@]}))]}" FINALV3_RR_T2 "${RR_TRANSFER_EVALS}" "$((SEED_BASE + 22))" "" "${transfer_overrides_2[@]}"
else
  idx=0
  for parent in "${transfer_parents[@]}"; do
    idx=$((idx + 1))
    gpu="${GPUS[$(((idx - 1) % ${#GPUS[@]}))]}"
    phase="FINALV3_RR_T${idx}"
