...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v3_${PHASE%%_*}_train_${EXECUTION}"
fi
if [ -z "$EXP_DESC" ]; then
  EXP_DESC="Single-train run for reproducibility/smoke with fixed hyperparameters."
fi
if [ -z "$EXP_FOCUS" ]; then
  EXP_FOCUS="fmoe_stage_execution_mode,fmoe_v2_layout_id,learning_rate,weight_decay,train_batch_size,eval_batch_size"
fi
...
LOG_FILE_PATH="$(run_make_log_path fmoe_v3 train "$DATASET" "FeaturedMoE_v3_${EXECUTION}" "$GPU_ID" "$PHASE")"

cmd=(
  "$PY_BIN" recbole_train.py
  "model=featured_moe_v3"
  "dataset=${DATASET}"
  "eval_mode=session"
...
RUN_ID="$(run_tracker_start \
  ...
  --phase "$PHASE" \
  --dataset "$DATASET" \
  --model "FeaturedMoE_v3_${EXECUTION}" \
  --exp-name "$EXP_NAME" \
  --exp-desc "$EXP_DESC" \
  --exp-focus "$EXP_FOCUS" \
  --cmd "$CMD_STR" \
