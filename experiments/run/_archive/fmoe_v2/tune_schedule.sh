...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v2_${PHASE%%_*}_schedule_${MODE}"
fi
if [ -z "$EXP_DESC" ]; then
...
LOG_FILE_PATH="$(run_make_log_path fmoe_v2 schedule "$DATASET" "FeaturedMoE_v2_${MODE}" "$GPU_ID" "$PHASE")"
