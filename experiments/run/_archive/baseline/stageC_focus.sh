...
TUNE_EPOCHS_DEFAULT="44"
TUNE_EPOCHS_DUOREC="30"
TUNE_EPOCHS_FAME="36"

TUNE_PATIENCE_DEFAULT="6"
TUNE_PATIENCE_DUOREC="4"
TUNE_PATIENCE_FAME="5"
...
Usage: $0 [--datasets lastfm0.03,amazon_beauty] [--models sasrec,gru4rec,duorec,difsr,fame]
          [--profiles C1,C2,C3,C4,C5,C6] [--b-topk 2]
          [--gpus 0,1,2,3] [--seeds 1] [--seed-base 180000]
          [--max-evals-default 6] [--max-evals-duorec 4] [--max-evals-fame 5]
          [--tune-epochs-default 44] [--tune-epochs-duorec 30] [--tune-epochs-fame 36]
          [--tune-patience-default 6] [--tune-patience-duorec 4] [--tune-patience-fame 5]
...
CMD=(
  "${PYTHON_BIN}"
  "${SCRIPT_DIR}/run_stageC_focus.py"
  --datasets "${DATASETS}"
  --models "${MODELS}"
  --profiles "${PROFILES}"
  --b-topk "${B_TOPK}"
...
echo "[All Done] baseline StageC focus completed: ${DATASETS}"
