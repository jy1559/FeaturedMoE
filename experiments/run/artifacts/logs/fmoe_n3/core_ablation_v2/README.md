...
- 추정 총 예산: `4479.6 GPU-min`
- 4 GPU 기준 추정 wall time: `18.66 h`
...
- GPU 0: `1042.6 GPU-min` 예상
  - `G01` `P01` `plain_c3_three_layer` (추천 6 eval, 50epoch 환산 8.8분)
  - `G02` `D14` `dense_gated_macro_mid` (추천 6 eval, 50epoch 환산 8.7분)
  - `G03` `R30` `full_rule_soft` (추천 4 eval, 50epoch 환산 60.4분)
  - `G04` `R32` `full_moe_feature_only` (추천 3 eval, 50epoch 환산 65.4분)
  - `G05` `E40` `full_moe_both_complex_all` (추천 3 eval, 50epoch 환산 65.4분)
  - `G06` `X61` `m22_tempo_memory_only` (추천 4 eval, 50epoch 환산 60.4분)
  - `G07` `C71` `dense_plain_param_match_c70` (추천 6 eval, 50epoch 환산 10.4분)
- GPU 1: `1177.5 GPU-min` 예상
  - `G01` `D10` `dense_plain_macro_mid` (추천 6 eval, 50epoch 환산 8.7분)
  - `G02` `D15` `dense_gated_macro_mid_micro` (추천 6 eval, 50epoch 환산 9.6분)
  - `G03` `M20` `macro_only_moe_both` (추천 4 eval, 50epoch 환산 25.4분)
  - `G04` `R31` `full_moe_hidden_only` (추천 4 eval, 50epoch 환산 60.4분)
  - `G05` `E41` `full_moe_both_complex_macro` (추천 4 eval, 50epoch 환산 60.4분)
  - `G06` `X60` `m22_macro_window_10` (추천 4 eval, 50epoch 환산 60.4분)
  - `G07` `C70` `m22_expert_scale_3` (추천 4 eval, 50epoch 환산 60.4분)
- GPU 2: `1046.9 GPU-min` 예상
  - `G01` `D11` `dense_plain_macro_mid_micro` (추천 6 eval, 50epoch 환산 8.7분)
  - `G02` `D12` `dense_film_macro_mid` (추천 6 eval, 50epoch 환산 8.7분)
  - `G03` `M22` `full_moe_both_anchor` (추천 4 eval, 50epoch 환산 60.4분)
  - `G04` `R34` `hybrid_macro_learn_mid_micro_rule` (추천 4 eval, 50epoch 환산 60.4분)
  - `G05` `T50` `full_moe_both_mid_token` (추천 3 eval, 50epoch 환산 65.4분)
  - `G06` `T51` `full_moe_both_all_token` (추천 3 eval, 50epoch 환산 65.4분)
  - `G07` `C72` `dense_film_param_match_c70` (추천 6 eval, 50epoch 환산 11.2분)
- GPU 3: `1212.6 GPU-min` 예상
  - `G01` `P00` `plain_c2_one_layer` (추천 6 eval, 50epoch 환산 8.0분)
  - `G02` `D13` `dense_film_macro_mid_micro` (추천 6 eval, 50epoch 환산 9.6분)
  - `G03` `M21` `macro_mid_moe_both` (추천 4 eval, 50epoch 환산 42.9분)
  - `G04` `R33` `full_moe_hidden_gated_bias` (추천 4 eval, 50epoch 환산 60.4분)
  - `G05` `E42` `full_moe_both_complex_mid` (추천 4 eval, 50epoch 환산 60.4분)
  - `G06` `X62` `m22_len_30` (추천 3 eval, 50epoch 환산 70.4분)
  - `G07` `X63` `m22_topk_2` (추천 4 eval, 50epoch 환산 60.4분)

## 파일

- combo 상세 표: `experiments/run/artifacts/logs/fmoe_n3/core_ablation_v2/combo_reference.csv`
- 4 GPU 분배표: `experiments/run/artifacts/logs/fmoe_n3/core_ablation_v2/gpu_4way_plan.csv`
