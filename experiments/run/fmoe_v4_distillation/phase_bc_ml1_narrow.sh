...
TUNE_EPOCHS="60"
TUNE_PATIENCE="8"
B_PHASE_PREFIX="P3ROUTER"
C_PHASE_PREFIX="P3DISTILL"
LOG_WANDB="false"
...
Usage: $0 [--dataset movielens1m]
          ...
          [--b-phase-prefix P3ROUTER] [--c-phase-prefix P3DISTILL]
          [--log-wandb] [--dry-run]
USAGE
}
...
cmd_b=(
  bash "${SCRIPT_DIR}/phase_b_ml1_router_narrow.sh"
  --datasets "${DATASET}"
  --gpus "${B_GPUS}"
  --combos-per-gpu "2"
  --max-evals "${MAX_EVALS}"
...
cmd_c=(
  bash "${SCRIPT_DIR}/phase_c_ml1_distill_narrow.sh"
  --datasets "${DATASET}"
  --gpus "${C_GPUS}"
  --combos-per-gpu "2"
  --max-evals "${MAX_EVALS}"
