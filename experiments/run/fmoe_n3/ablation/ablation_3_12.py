#!/usr/bin/env python3
"""Launch KuaiRec-only A12 ablation v3 (12 settings) for FMoE_N3.

Matrix design:
- Router source decomposition (4)
- Router granularity decomposition (4)
- Feature injection decomposition (4)
"""
...
AXIS = "ablation_feature_add_v3_a12_kuairec_v3"
PHASE_ID = "P17C"
PHASE_NAME = "A12_ABLATION3_12"
RUN_STAGE = "wide"
AXIS_ID = "C"
AXIS_DESC = "kuairec_a12_ablation3_12"
...
def _manifest_path(args: argparse.Namespace) -> Path:
    ...
    return _phase_log_dir(args.dataset) / "ablation3_12_matrix.json"
...
def _setting(
    *,
    ...
    return {
        ...
        "setting_key": f"ABL3-{int(idx):02d}_{key}",
        "setting_desc": key,
        "setting_group": group,
...
def _build_settings(args: argparse.Namespace) -> list[Dict[str, Any]]:
    ...
    # 1) Router source decomposition (4)
    settings.append(
        _setting(
            idx=1,
            key="RS_ALL_FEATURE",
            group="router_source",
            detail="router source = feature for all stages",
            base=base,
            extra_overrides={"stage_router_source": _all_stage_map("feature")},
        )
    )
    settings.append(
        _setting(
            ...
            key="RS_ALL_HIDDEN",
            group="router_source",
            detail="router source = hidden for all stages",
            base=base,
            extra_overrides={"stage_router_source": _all_stage_map("hidden")},
        )
    )
    settings.append(
        _setting(
            idx=3,
            key="RS_MACRO_FEATURE_OTHERS_BOTH",
            group="router_source",
            detail="macro=feature, mid/micro=both",
            base=base,
            extra_overrides={"stage_router_source": {"macro": "feature", "mid": "both", "micro": "both"}},
        )
    )
    settings.append(
        _setting(
            idx=4,
            key="RS_MICRO_HIDDEN_OTHERS_BOTH",
            group="router_source",
            detail="micro=hidden, macro/mid=both",
            base=base,
            extra_overrides={"stage_router_source": {"macro": "both", "mid": "both", "micro": "hidden"}},
        )
    )

    # 2) Router granularity decomposition (4)
    settings.append(
        _setting(
            idx=5,
            key="RG_ALL_TOKEN",
            group="router_granularity",
            detail="router granularity = token for all stages",
            base=base,
            extra_overrides={"stage_router_granularity": _all_stage_map("token")},
        )
    )
    settings.append(
        _setting(
            ...
            key="RG_ALL_SESSION",
            group="router_granularity",
            detail="router granularity = session for all stages",
            base=base,
            extra_overrides={"stage_router_granularity": _all_stage_map("session")},
        )
    )
    settings.append(
        _setting(
            idx=7,
            key="RG_MACRO_TOKEN_MID_SESSION_MICRO_TOKEN",
            group="router_granularity",
            detail="macro=token, mid=session, micro=token",
            base=base,
            extra_overrides={"stage_router_granularity": {"macro": "token", "mid": "session", "micro": "token"}},
        )
    )
    settings.append(
        _setting(
            idx=8,
            key="RG_MACRO_SESSION_MID_TOKEN_MICRO_TOKEN",
            group="router_granularity",
            detail="macro=session, mid=token, micro=token",
            base=base,
            extra_overrides={"stage_router_granularity": {"macro": "session", "mid": "token", "micro": "token"}},
        )
    )

    # 3) Feature injection decomposition (4)
    settings.append(
        _setting(
            idx=9,
            key="FI_ALL_GATED_BIAS",
            group="feature_injection",
            detail="gated_bias injection on all stages",
            base=base,
            extra_overrides={"stage_feature_injection": _all_stage_map("gated_bias")},
        )
    )
    settings.append(
        _setting(
            idx=10,
            key="FI_MACRO_GATED_ONLY",
            group="feature_injection",
            detail="macro gated_bias only; mid/micro none",
            base=base,
            extra_overrides={"stage_feature_injection": {"macro": "gated_bias", "mid": "none", "micro": "none"}},
        )
    )
    settings.append(
        _setting(
            idx=11,
            key="FI_ALL_GROUP_GATED_BIAS",
            group="feature_injection",
            detail="group_gated_bias injection on all stages",
            base=base,
            extra_overrides={"stage_feature_injection": _all_stage_map("group_gated_bias")},
        )
    )
    settings.append(
        _setting(
            idx=12,
            key="FI_MACRO_GROUP_GATED_ONLY",
            group="feature_injection",
            detail="macro group_gated_bias only; mid/micro none",
            base=base,
            extra_overrides={"stage_feature_injection": {"macro": "group_gated_bias", "mid": "none", "micro": "none"}},
        )
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FMoE_N3 KuaiRec A12 ablation3-12 launcher")
    parser.add_argument("--dataset", default="KuaiRecLargeStrictPosV2_0.2")
    parser.add_argument("--gpus", default="0,1,2,3,4,5")
    parser.add_argument("--seeds", default="1")
    parser.add_argument("--seed-base", type=int, default=172000)
...
def _apply_smoke_mode(args: argparse.Namespace) -> None:
    ...
    if not args.only_setting:
        args.only_setting = "ABL3-01_RS_ALL_FEATURE,ABL3-09_FI_ALL_GATED_BIAS"
