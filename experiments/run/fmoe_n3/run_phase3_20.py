#!/usr/bin/env python3
"""Launch FMoE_N3 phase3 20-combo track.

Design:
- 4 structure families (top-3 from P2 + 1 surprising but decent)
- 5 combos per structure = 20 total
- Per structure: 3 conservative variants + 2 unusual variants
"""
...
DEFAULT_LR_FOCUSED = (2.0e-4, 2.0e-3)
...
def runtime_bucket(combo: dict) -> str:
    if int(combo.get("MAX_ITEM_LIST_LENGTH", 20)) >= 50:
        return "heavy"
    if int(combo.get("embedding_size", 128)) >= 192:
        return "heavy"
...
def estimated_runtime_profile(combo: dict) -> dict:
    bucket = runtime_bucket(combo)
    trial_min = 1.5 if bucket == "heavy" else 1.1 if bucket == "medium-heavy" else 1.0
    fixed_overhead = 0.4
...
def build_combos(dataset: str) -> list[dict]:
    ...
    lr_focused = DEFAULT_LR_FOCUSED

    combos: list[dict] = []

    # ------------------------------------------------------------------
    # Structure S1 (top performer): Standard router + all gated_bias (PA05)
    # ------------------------------------------------------------------
    combos.extend([
        make_combo("P3S1_01", combo_family="S1", combo_role="core", desc="s1_base_all_gated_bias",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1], stage_feature_injection=_all_stage_map("gated_bias")),
        make_combo("P3S1_02", combo_family="S1", combo_role="core", desc="s1_macro_complex_plus_gated",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_feature_encoder_mode={"macro": "complex", "mid": "linear", "micro": "linear"},
                   stage_feature_injection=_all_stage_map("gated_bias")),
        make_combo("P3S1_03", combo_family="S1", combo_role="core", desc="s1_routerdim96_feat24",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1], stage_feature_injection=_all_stage_map("gated_bias"),
                   d_router_hidden=96, d_feat_emb=24, d_expert_hidden=144),
        make_combo("P3S1_04", combo_family="S1", combo_role="unusual", desc="s1_topk1_all_token",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1], stage_feature_injection=_all_stage_map("gated_bias"),
                   stage_router_granularity=_all_stage_map("token"), gate_entropy_lambda=2e-4),
        make_combo("P3S1_05", combo_family="S1", combo_role="unusual", desc="s1_len50_expertscale4",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["len50_train_bs"], eval_bs=prof["len50_eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1], max_len=50, expert_scale=4,
                   stage_feature_injection=_all_stage_map("gated_bias"), balance_loss_lambda=0.006),
    ])

    # ------------------------------------------------------------------
    # Structure S2 (top + theory): Factored router + group_gated_bias (PD04)
    # ------------------------------------------------------------------
    combos.extend([
        make_combo("P3S2_01", combo_family="S2", combo_role="core", desc="s2_base_factored_groupinj",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
        ...
        make_combo("P3S2_02", combo_family="S2", combo_role="core", desc="s2_factored_plus_group_balance",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_router_type=_all_stage_map("factored"),
                   stage_feature_injection=_all_stage_map("group_gated_bias"),
                   stage_router_source=_all_stage_map("both"), factored_group_balance_lambda=0.003),
        make_combo("P3S2_03", combo_family="S2", combo_role="core", desc="s2_factored_plus_prior_align",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_router_type=_all_stage_map("factored"),
                   stage_feature_injection=_all_stage_map("group_gated_bias"),
                   stage_router_source=_all_stage_map("both"), group_prior_align_lambda=5e-4),
        make_combo("P3S2_04", combo_family="S2", combo_role="unusual", desc="s2_pure_feature_factored_groupinj",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   stage_router_type=_all_stage_map("factored"),
                   stage_feature_injection=_all_stage_map("group_gated_bias"),
                   stage_router_source=_all_stage_map("feature"), d_router_hidden=80),
        make_combo("P3S2_05", combo_family="S2", combo_role="unusual", desc="s2_two_group_ablation",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   stage_router_type=_all_stage_map("factored"),
                   stage_feature_injection=_all_stage_map("group_gated_bias"),
                   ...
                   group_coverage_lambda=0.3),
    ])

    # ------------------------------------------------------------------
    # Structure S3 (top): feature-source routing (PA_FS)
    # ------------------------------------------------------------------
    combos.extend([
        make_combo("P3S3_01", combo_family="S3", combo_role="core", desc="s3_base_feature_source",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1], stage_router_source=_all_stage_map("feature")),
        make_combo("P3S3_02", combo_family="S3", combo_role="core", desc="s3_feature_source_plus_gated",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_router_source=_all_stage_map("feature"),
                   stage_feature_injection=_all_stage_map("gated_bias")),
        make_combo("P3S3_03", combo_family="S3", combo_role="core", desc="s3_feature_source_complex_encoder",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_router_source=_all_stage_map("feature"),
                   stage_feature_encoder_mode=_all_stage_map("complex"), d_feat_emb=24),
        make_combo("P3S3_04", combo_family="S3", combo_role="unusual", desc="s3_feature_source_factored",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   stage_router_source=_all_stage_map("feature"),
                   stage_router_type=_all_stage_map("factored"),
                   group_prior_align_lambda=5e-4),
        make_combo("P3S3_05", combo_family="S3", combo_role="unusual", desc="s3_feature_source_all_token_lowrouter",
                   layer_layout=["macro", "mid", "micro"], train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   stage_router_source=_all_stage_map("feature"),
                   stage_router_granularity=_all_stage_map("token"), d_router_hidden=48),
    ])

    # ------------------------------------------------------------------
    # Structure S4 (surprise): deep layer prefix (PA_L08)
    # ------------------------------------------------------------------
    combos.extend([
        make_combo("P3S4_01", combo_family="S4", combo_role="core", desc="s4_base_deep_layer_prefix",
                   layer_layout=["layer", "layer", "macro", "mid", "micro"],
                   train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1]),
        make_combo("P3S4_02", combo_family="S4", combo_role="core", desc="s4_prefix_plus_gated_bias",
                   layer_layout=["layer", "layer", "macro", "mid", "micro"],
                   train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   stage_feature_injection=_all_stage_map("gated_bias")),
        make_combo("P3S4_03", combo_family="S4", combo_role="core", desc="s4_prefix_routerdim96_feat24",
                   layer_layout=["layer", "layer", "macro", "mid", "micro"],
                   train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_focused[0], lr_max=lr_focused[1],
                   d_router_hidden=96, d_feat_emb=24, d_expert_hidden=144),
        make_combo("P3S4_04", combo_family="S4", combo_role="unusual", desc="s4_ffn_bridge_variant",
                   layer_layout=["layer", "macro_ffn", "mid_ffn", "micro_ffn"],
                   train_bs=prof["train_bs"], eval_bs=prof["eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   stage_feature_encoder_mode={"macro": "complex", "mid": "linear", "micro": "linear"}),
        make_combo("P3S4_05", combo_family="S4", combo_role="unusual", desc="s4_hidden_router_len50",
                   layer_layout=["layer", "layer", "macro", "mid", "micro"],
                   train_bs=prof["len50_train_bs"], eval_bs=prof["len50_eval_bs"],
                   lr_min=lr_base[0], lr_max=lr_base[1],
                   max_len=50, stage_router_source=_all_stage_map("hidden"), expert_scale=2),
    ])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FMoE_N3 phase3 20-combo launcher")
    ...
    parser.add_argument("--seed-base", type=int, default=10300)
    parser.add_argument("--use-recommended-budget", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--manifest-out", default="")
    parser.add_argument("--only", default="", help="Comma-separated list of combo_ids")
    parser.add_argument("--family", default="", help="Comma-separated families e.g. S1,S2")
    ...
    bins = plan_gpu_bins(combos, gpus, cost_key="estimated_50ep_single_eval_min")
    ...
    if args.dry_run:
        ...
        for gpu_id, combo, _, _ in all_assigned:
    ...
    while queue or active:
        ...
        for gpu_id in gpus:
            if gpu_id not in active and queue:
                gpu_id_match = None
                for i, (g, _, _, _) in enumerate(queue):
                    if g == gpu_id:
                        gpu_id_match = i
                        break
    ...
    print(f"\\n[done] Phase 3 complete - {len(all_assigned)} combos launched.")
