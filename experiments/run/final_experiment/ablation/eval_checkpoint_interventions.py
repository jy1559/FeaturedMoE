#!/usr/bin/env python3
"""Run eval-only feature perturb interventions for one exported checkpoint."""
...
from common import build_eval_config_from_result_payload, q5_intervention_specs  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Eval one checkpoint under Q5 intervention settings.")
    ...
    manifest_path = bundle_root / "intervention_manifest.csv"

    specs = list(q5_intervention_specs())
    if args.max_interventions and int(args.max_interventions) > 0:
        specs = specs[: int(args.max_interventions)]

    manifest_rows: list[dict[str, str]] = []
    for idx, spec in enumerate(specs, start=1):
        intervention = str(spec["intervention"])
        print(f"[{idx}/{len(specs)}] intervention={intervention}")
        eval_cfg = deepcopy(base_cfg)
        eval_cfg.update(deepcopy(spec.get("overrides") or {}))
        eval_cfg["run_phase"] = f"Q5_INTERVENTION_{_sanitize(intervention)}"
        eval_cfg["case_eval_label"] = intervention
        eval_cfg["case_eval_scope"] = "original"
        ...
        try:
            ...
            result_payload["intervention_group"] = str(spec.get("group", intervention))
            result_payload["target_family"] = ",".join(spec.get("overrides", {}).get("feature_perturb_family", []) or [])

            if logging_dir:
                result_file = str(Path(logging_dir) / "intervention_result.json")
            else:
            ...
            error = str(exc)
            print(f"[WARN] intervention={intervention} failed: {exc}")
        ...
        manifest_rows.append(
            {
                ...
                "intervention_group": str(spec.get("group", intervention)),
                "target_family": ",".join(spec.get("overrides", {}).get("feature_perturb_family", []) or []),
