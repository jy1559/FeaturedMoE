...
run_build() {
  if [[ "${SKIP_BUILD_SPACE}" == "1" ]]; then
    echo "[final_experiment][server_a] build-space skipped (SKIP_BUILD_SPACE=1)"
    return
  fi
  echo "[final_experiment][server_a] build-space"
...
run_build_with_args() {
  ...
  echo "[final_experiment][server_a] build-space extra_args=${EXTRA_ARGS[*]:-}"
  python3 "${SCRIPT_DIR}/build_space_manifest.py" "${EXTRA_ARGS[@]}"
}

run_stage1() {
  echo "[final_experiment][server_a] stage1 datasets=${DATASETS} models=${BASELINE_MODELS} gpus=${GPU_LIST} max_run_hours=${MAX_RUN_HOURS} oom_retry_limit=${OOM_RETRY_LIMIT}"
...
run_stage2() {
  echo "[final_experiment][server_a] stage2 datasets=${DATASETS} models=${BASELINE_MODELS} gpus=${GPU_LIST} max_run_hours=${MAX_RUN_HOURS} oom_retry_limit=${OOM_RETRY_LIMIT}"
...
run_stage3() {
  echo "[final_experiment][server_a] stage3 datasets=${DATASETS} models=${BASELINE_MODELS} gpus=${GPU_LIST} max_run_hours=${MAX_RUN_HOURS} oom_retry_limit=${OOM_RETRY_LIMIT}"
