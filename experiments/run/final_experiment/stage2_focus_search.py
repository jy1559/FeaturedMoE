...
from common import (
    ...
    now_utc,
    parse_csv_list,
    route_stage2_refine_space,
    run_jobs,
    sanitize_token,
    select_route_stage2_families,
    stage_manifest_path,
    top_unique_trials,
...
def build_rows(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for dataset in datasets:
        ...
        if ROUTE_MODEL in models:
            ...
            selected_route_rows = select_route_stage2_families(dataset, route_index.get(dataset, {}) or {}, route_candidates)
            for rank, item in enumerate(selected_route_rows, start=1):
                base_row = dict(item.get("manifest_row", {}) or {})
                if not base_row:
                    continue
                top_trial = top_unique_trials(
                    item["payload"],
                    top_k=int(route_index.get(dataset, {}).get("stage2_top_trials_per_family", 2) or 2),
                )
                ...
                rows.append(
                    {
                        ...
                        "selection_reason": item.get("selection_reason", "stage1_winner"),
                        "family_role": base_row.get("family_role", ""),
                        "history_valid": base_row.get("history_valid", 0.0),
                        "history_test": base_row.get("history_test", 0.0),
                        "search_space": route_stage2_refine_space(
                            dataset,
                            dict(base_row.get("search_space") or {}),
                            dict(base_row.get("fixed_context") or {}),
                            configs,
                        ),
