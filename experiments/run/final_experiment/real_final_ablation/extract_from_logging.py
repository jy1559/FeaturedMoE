...
def _get_routing_profile(run: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for stage_name, sdata in sm.items():
        group_routing = dict(sdata.get("group_routing", {}) or {})
        family_names = list(group_routing.get("group_names", []) or sdata.get("family_names", []) or [])
        group_share = list(group_routing.get("group_share", []) or [])
        if not family_names or not group_share:
            continue
        for fname, share in zip(family_names, group_share):
            rows.append({
                "stage_name": stage_name,
                "routed_family": fname.lower(),
                "usage_share": round(float(share), 4),
            })
    return rows
