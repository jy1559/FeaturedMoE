...
PROFILES="F1,F2,F3,F4,F5,F6,F7,F8"
E_TOPK="3"
GPU_LIST="0,1,2,3"
SEEDS="1"
SEED_BASE="210000"
...
TUNE_EPOCHS_DEFAULT="72"
TUNE_EPOCHS_DUOREC="56"
TUNE_EPOCHS_FAME="64"

TUNE_PATIENCE_DEFAULT="9"
TUNE_PATIENCE_DUOREC="7"
...
usage() {
  cat <<USAGE
Usage: $0 [--datasets lastfm0.03,amazon_beauty] [--models gru4rec,fame,duorec]
          [--profiles F1,F2,F3,F4,F5,F6,F7,F8] [--e-topk 3]
          [--gpus 0,1,2,3] [--seeds 1] [--seed-base 210000]
          [--max-evals-default 12] [--max-evals-duorec 10] [--max-evals-fame 12]
          [--tune-epochs-default 72] [--tune-epochs-duorec 56] [--tune-epochs-fame 64]
          [--tune-patience-default 9] [--tune-patience-duorec 7] [--tune-patience-fame 8]
...
CMD=(
  "${PYTHON_BIN}"
  "${SCRIPT_DIR}/run_stageF_tail_boost.py"
  --datasets "${DATASETS}"
  --models "${MODELS}"
  --profiles "${PROFILES}"
  --b-topk "${E_TOPK}"
...
echo "[All Done] baseline StageF tail-boost completed: ${DATASETS}"
