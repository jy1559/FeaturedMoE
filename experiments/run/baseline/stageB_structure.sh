...
TUNE_EPOCHS_DEFAULT="40"
TUNE_EPOCHS_DUOREC="28"
TUNE_EPOCHS_FAME="34"
...
Usage: $0 [--datasets lastfm0.03,amazon_beauty] [--models sasrec,gru4rec,duorec,difsr,fame]
          [--profiles B1,B2,B3,B4,B5,B6]
          [--gpus 0,1,2,3] [--seeds 1] [--seed-base 170000]
          [--max-evals-default 6] [--max-evals-duorec 4] [--max-evals-fame 5]
          [--tune-epochs-default 40] [--tune-epochs-duorec 28] [--tune-epochs-fame 34]
...
CMD=(
  "${PYTHON_BIN}"
  "${SCRIPT_DIR}/run_stageB_structure.py"
  --datasets "${DATASETS}"
  --models "${MODELS}"
  --profiles "${PROFILES}"
  --gpus "${GPU_LIST}"
...
echo "[All Done] baseline StageB structure completed: ${DATASETS}"
