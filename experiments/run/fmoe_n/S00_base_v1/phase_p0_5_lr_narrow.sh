...
if [ -z "$MANIFEST_PATH" ]; then
  MANIFEST_PATH="${INV_DIR}/p0_manifest_${STATE_TAG}_latest.json"
fi
[ -f "$MANIFEST_PATH" ] || { echo "manifest not found: ${MANIFEST_PATH}" >&2; exit 1; }

RUN_TAG="$(run_timestamp)"
WORK_DIR="${INV_DIR}/p05_${STATE_TAG}_${RUN_TAG}"
...
if [ -z "$MANIFEST_OUT" ]; then
  MANIFEST_OUT="${INV_DIR}/p05_manifest_${STATE_TAG}_${RUN_TAG}.json"
fi
LATEST_MANIFEST="${INV_DIR}/p05_manifest_${STATE_TAG}_latest.json"

python3 - <<'PY' "$MANIFEST_PATH" "$PLAN_PATH" "$GPU_LIST" "$AUTO_PRUNE_RELATIVE" "$SIGMA_THRESHOLD" "$MAX_DROP_KUAI" "$MAX_DROP_LFM" "$STATE_TAG"
import json
...
state_tag_override = sys.argv[8].strip()
gpus = [x.strip() for x in gpu_csv.split(",") if x.strip()]

manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
combos = manifest.get("combos", [])
state_tag = (state_tag_override or str(manifest.get("axis", "")).strip() or "hparam").lower()
  ...
  for row in "${wave_rows[@]}"; do
    ...
    (
      ...
      cmd=(
        ...
        --combo-desc "p05_followup_${combo_id}"
