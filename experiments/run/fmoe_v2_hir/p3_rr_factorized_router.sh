...
SEED_BASE="1020"
PHASE_PREFIX="P3RGI"
CATALOG_PROFILE="router6"
ROUTING_PROFILE="gate_aux"
SCHEDULE_PRESET="off"
LOG_WANDB="false"
...
Usage: $0 [--datasets retail_rocket] [--gpus 0,1,2,3]
          [--catalog-profile router6|router8] [--routing-profile gate_aux|distill]
          [--combo-catalog spec] [--combos-per-gpu N]
          [--max-evals N] [--tune-epochs N] [--tune-patience N]
          [--phase-prefix P3RGI] [--seed-base N]
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --routing-profile) ROUTING_PROFILE="$2"; shift 2 ;;
...
build_combo_catalog() {
  case "$CATALOG_PROFILE" in
    router6) build_router6_combo_catalog ;;
    router8) build_router8_combo_catalog ;;
    *)
...
lr_space_for_combo() {
  ...
  if [ "$train_bs" -le 3072 ] || [ "$drouter" -ge 96 ] || [ "$dexp" -ge 192 ]; then
    echo "2.5e-4,3.5e-4,4.5e-4,5.5e-4,6.5e-4"
  elif [ "$emb" -ge 160 ] || [ "$dfeat" -ge 24 ]; then
    echo "3e-4,4e-4,5e-4,6e-4,7e-4"
  else
    echo "3.5e-4,4.5e-4,5.5e-4,6.5e-4"
  fi
}
...
apply_rr_oom_cap() {
  ...
  if [ "$emb" -ge 192 ] || [ "$dexp" -ge 224 ] || [ "$drouter" -ge 96 ] || [ "$expert_scale" -ge 4 ] || [ "$dfeat" -ge 32 ]; then
...
for path in root.glob(f"{dataset}_FeaturedMoE_v2_HiR_*.json"):
    ...
    if phase_prefixes and not any(pref in name for pref in phase_prefixes):
        continue
...
routing_overrides() {
  local routing_profile="$1"
  local default_fspec="$2"

  case "$routing_profile" in
    gate_aux)
      cat <<EOF
fmoe_v2_feature_spec_aux_lambda=${default_fspec}
++search.fmoe_v2_feature_spec_aux_lambda=[1e-4,3e-4,7e-4]
fmoe_v2_feature_spec_stages=[mid]
++search.fmoe_v2_feature_spec_stages=[[mid],[mid,micro]]
expert_top_k=1
++search.expert_top_k=[1,2]
group_top_k=0
...
EOF
    ...
    distill)
      cat <<EOF
fmoe_v2_feature_spec_aux_lambda=${default_fspec}
++search.fmoe_v2_feature_spec_aux_lambda=[3e-4,7e-4]
fmoe_v2_feature_spec_stages=[mid]
++search.fmoe_v2_feature_spec_stages=[[mid]]
expert_top_k=1
++search.expert_top_k=[1]
group_top_k=0
++search.group_top_k=[0]
router_distill_enable=true
++search.router_distill_enable=[true]
router_distill_lambda=2e-3
++search.router_distill_lambda=[2e-3,5e-3]
router_distill_temperature=1.5
++search.router_distill_temperature=[1.2,1.5]
router_distill_until=0.2
++search.router_distill_until=[0.15,0.2]
EOF
      ;;
    *)
...
append_override_lines() {
  local -n target_ref=$1
  local line
  while IFS= read -r line; do
    [ -z "$line" ] && continue
    target_ref+=(--override "$line")
  done
}
...
on_interrupt() {
  INTERRUPTED="true"
  echo "[INTERRUPT] stopping RR factorized P3 router workers..."
  local p
...
for ds in "${DATASET_ARR[@]}"; do
  echo "=== [${ds}] P3 factorized RR router combos=${COVERED_COMBOS}/${COMBO_N} gpus=${#GPUS[@]} cpg=${COMBOS_PER_GPU} profile=${ROUTING_PROFILE} ==="
  gpu_plan=()
  for gidx in "${!GPUS[@]}"; do
    gpu="${GPUS[$gidx]}"
  ...
  for gidx in "${!GPUS[@]}"; do
    ...
    (
      ...
      for ((slot=0; slot<COMBOS_PER_GPU; slot++)); do
        ...
        phase="${PHASE_PREFIX}_G${gpu}_C$(printf '%02d' "$combo_idx")_${execution}_L${layout_id}_E${emb}_F${dfeat}_H${dexp}_R${drouter}_B${train_bs}_${ROUTING_PROFILE}"
        exp_name="${PHASE_PREFIX}_${ds}_${execution}_layout${layout_id}_${ROUTING_PROFILE}"
        exp_desc="${PHASE_PREFIX}: RR factorized-router P3. Fix top layout/dim seeds and tune expert sparsity, feature specialization, and optional rule-teacher distillation."
        exp_focus="fmoe_stage_execution_mode,fmoe_v2_layout_id,embedding_size,d_feat_emb,d_expert_hidden,d_router_hidden,expert_scale,train_batch_size,eval_batch_size,learning_rate,weight_decay,balance_loss_lambda,expert_top_k,fmoe_v2_feature_spec_aux_lambda,fmoe_v2_feature_spec_stages,router_distill_enable,router_distill_lambda"
        parent_result="$(find_parent_result "$ds" "$layout_id" "$emb" "$dfeat" "$dexp" "$drouter" "$train_bs" "$PARENT_PHASE_PREFIXES")"

        echo "[${PHASE_PREFIX}][${ds}] gpu=${gpu} combo=${combo_idx}/${COMBO_N} exec=${execution} layout=L${layout_id} emb=${emb} dfeat=${dfeat} dexp=${dexp} drouter=${drouter} scale=${expert_scale} bs=${train_bs}/${eval_bs} routing=${ROUTING_PROFILE}"
        ...
        append_override_lines cmd < <(routing_overrides "$ROUTING_PROFILE" "$fspec_value")
        ...
        if [ "$rc" -ne 0 ]; then
          ...
          if is_oom_log "$log_path" && [ "$train_bs" -ge $((OOM_RETRY_MIN_TRAIN_BS * 2)) ]; then
            ...
            append_override_lines retry_cmd < <(routing_overrides "$ROUTING_PROFILE" "$fspec_value")
            if [ -n "$parent_result" ]; then
