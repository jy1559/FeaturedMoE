...
root = base / 'fmoe_v2_hir'
if not root.exists():
    print('')
    raise SystemExit

cands = []
for p in root.glob(f"{ds}_FeaturedMoE_v2_HiR*.json"):
    try:
...
for i in "${!DS_ARR[@]}"; do
  ...
  p1_result="$(best_fmoe_v2_hir_result "$ds" hparam P1)"
  ...
  p2_result="$(best_fmoe_v2_hir_result "$ds" layout P2)"
  ...
  p3_result="$(best_fmoe_v2_hir_result "$ds" hparam P3)"
