...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v2_hir_${PHASE%%_*}_train_${EXECUTION}"
fi
if [ -z "$EXP_DESC" ]; then
...
LOG_FILE_PATH="$(run_make_log_path fmoe_v2_hir train "$DATASET" "FeaturedMoE_v2_HiR_${EXECUTION}" "$GPU_ID" "$PHASE")"

cmd=(
  "$PY_BIN" recbole_train.py
  "model=featured_moe_v2_hir"
  "dataset=${DATASET}"
  "eval_mode=session"
