...
if [ -z "$EXP_NAME" ]; then
  EXP_NAME="fmoe_v2_hir_${PHASE%%_*}_schedule_${MODE}"
fi
if [ -z "$EXP_DESC" ]; then
...
LOG_FILE_PATH="$(run_make_log_path fmoe_v2_hir schedule "$DATASET" "FeaturedMoE_v2_HiR_${MODE}" "$GPU_ID" "$PHASE")"
...
RUN_ID="$(run_tracker_start \
  ...
  --phase "$PHASE" \
  --dataset "$DATASET" \
  --model "FeaturedMoE_v2_HiR_${MODE}" \
  --exp-name "$EXP_NAME" \
  --exp-desc "$EXP_DESC" \
  --exp-focus "$EXP_FOCUS" \
  --cmd "$CMD_STR" \
