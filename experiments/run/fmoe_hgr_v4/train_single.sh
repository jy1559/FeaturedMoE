...
[ -z "$EXP_NAME" ] && EXP_NAME="fmoe_hgr_v4_${PHASE%%_*}_single"
[ -z "$EXP_DESC" ] && EXP_DESC="HGRv4 single run with feature-aware outer router and stat-soft inner teacher."
[ -z "$EXP_FOCUS" ] && EXP_FOCUS="arch_layout_id,group_router_mode,embedding_size,d_expert_hidden,d_router_hidden,expert_scale,inner_rule_mode,learning_rate"

LOG_FILE_PATH="$(run_make_phase_log_path fmoe_hgr_v4 train "$DATASET" "FeaturedMoE_HGRv4" "$PHASE")"
