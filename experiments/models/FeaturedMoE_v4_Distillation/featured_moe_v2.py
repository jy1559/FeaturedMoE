"""FeaturedMoE_v4_Distillation model."""
...
class FeaturedMoE_V4_Distillation(SequentialRecommender):
    """FeaturedMoE_v4_Distillation: flat router with direct 12-way rule teachers."""
    ...
    def __init__(self, config, dataset):
        ...
        rule_router_variant = self.rule_router_cfg.get("variant", "ratio_bins")
        if isinstance(rule_router_variant, (list, tuple)):
            rule_router_variant = rule_router_variant[0] if len(rule_router_variant) > 0 else "ratio_bins"
        rule_router_variant = str(rule_router_variant).lower().strip()
        if rule_router_variant not in {"ratio_bins", "teacher_gls"}:
            raise ValueError(
                "rule_router.variant must be one of ['ratio_bins','teacher_gls'], "
                f"got {rule_router_variant}"
            )
        self.router_design = str(resolver.get("router_design", "flat_legacy")).lower().strip()
        if self.router_design not in {"flat_legacy", "flat_clone_residual12"}:
            raise ValueError(
                "FeaturedMoE_v4_Distillation supports router_design in ['flat_legacy','flat_clone_residual12'], "
                f"got {self.router_design}"
            )
        self.router_group_bias_scale = float(resolver.get("router_group_bias_scale", 0.5))
        self.router_clone_residual_scale = float(resolver.get("router_clone_residual_scale", 0.20))
        ...
        self.teacher_design = str(resolver.get("teacher_design", "none")).lower().strip()
        self.teacher_delivery = str(resolver.get("teacher_delivery", "none")).lower().strip()
        self.teacher_stage_mask = str(resolver.get("teacher_stage_mask", "all")).lower().strip()
        self.teacher_kl_lambda = float(resolver.get("teacher_kl_lambda", 0.0))
        self.teacher_bias_scale = float(resolver.get("teacher_bias_scale", 0.0))
        self.teacher_temperature = float(resolver.get("teacher_temperature", 1.5))
        self.teacher_until = float(resolver.get("teacher_until", 0.25))
        self.teacher_stat_sharpness = float(resolver.get("teacher_stat_sharpness", 16.0))
        valid_teacher_designs = {
            "none",
            "group_local_stat12",
            "group_comp_stat12",
            "group_comp_shape12",
        ...
        valid_teacher_delivery = {"none", "distill_kl", "fused_bias", "distill_and_fused_bias"}
        valid_teacher_stage_masks = {"all", "mid_micro_only"}
        if self.teacher_design not in valid_teacher_designs:
            raise ValueError(
                f"teacher_design must be one of {sorted(valid_teacher_designs)}, got {self.teacher_design}"
            )
        if self.teacher_delivery not in valid_teacher_delivery:
            raise ValueError(
                f"teacher_delivery must be one of {sorted(valid_teacher_delivery)}, got {self.teacher_delivery}"
            )
        if self.teacher_stage_mask not in valid_teacher_stage_masks:
            raise ValueError(
                f"teacher_stage_mask must be one of {sorted(valid_teacher_stage_masks)}, got {self.teacher_stage_mask}"
            )
        ...
        self.stage_group_feature_indices = {
            stage_name: [
                [int(col2idx[f]) for f in feats if f in col2idx]
        ...
        self.stage_executor = StageExecutorV2(
            ...
            router_group_bias_scale=self.router_group_bias_scale,
            router_clone_residual_scale=self.router_clone_residual_scale,
            teacher_design=self.teacher_design,
            teacher_delivery=self.teacher_delivery,
            teacher_stage_mask=self.teacher_stage_mask,
            teacher_bias_scale=self.teacher_bias_scale,
            teacher_stat_sharpness=self.teacher_stat_sharpness,
        ...
        try:
            ...
            config["router_group_bias_scale"] = float(self.router_group_bias_scale)
            config["router_clone_residual_scale"] = float(self.router_clone_residual_scale)
            ...
            config["teacher_design"] = self.teacher_design
            config["teacher_delivery"] = self.teacher_delivery
            config["teacher_stage_mask"] = self.teacher_stage_mask
            config["teacher_kl_lambda"] = float(self.teacher_kl_lambda)
            config["teacher_bias_scale"] = float(self.teacher_bias_scale)
            config["teacher_temperature"] = float(self.teacher_temperature)
            config["teacher_until"] = float(self.teacher_until)
            config["teacher_stat_sharpness"] = float(self.teacher_stat_sharpness)
        except Exception:
            pass
        ...
        logger.info(
            "FeaturedMoE_v4_Distillation init: layout_id=%s execution=%s global_pre=%s global_post=%s boundaries=%s total_moe_blocks=%s "
            ...
            "router_impl=%s router_impl_by_stage=%s router_design=%s teacher_design=%s teacher_delivery=%s "
            "teacher_stage_mask=%s teacher_kl_lambda=%s teacher_bias_scale=%s rule_router=%s",
            self.layout_id,
            ...
            self.router_design,
            self.teacher_design,
            self.teacher_delivery,
            self.teacher_stage_mask,
            self.teacher_kl_lambda,
            self.teacher_bias_scale,
    ...
    def forward(self, item_seq, item_seq_len, feat=None):
        ...
        if feat is not None:
            tokens, gate_weights, gate_logits, stage_merge_weights, stage_merge_logits, router_stage_aux = self.stage_executor(
                hidden=tokens,
                item_seq=item_seq,
                feat=feat,
        ...
        aux_data = {
            ...
            "router_stage_aux": router_stage_aux,
    ...
    def calculate_loss(self, interaction):
        ...
        if self.use_aux_loss:
            ...
            aux_loss = aux_loss + compute_teacher_distill_aux_loss(
                router_stage_aux=aux_data.get("router_stage_aux", {}),
                item_seq_len=item_seq_len,
                teacher_delivery=self.teacher_delivery,
                teacher_kl_lambda=self.teacher_kl_lambda,
                teacher_temperature=self.teacher_temperature,
                progress=float(self._schedule_epoch) / float(max(self._schedule_total_epochs - 1, 1)),
                until=self.teacher_until,
                device=ce_loss.device,
            )
