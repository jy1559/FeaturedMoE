...
class StageExecutorV2(nn.Module):
        ...
        *,
        ...
        for stage_name in self.stage_order:
            ...
            cfg = StageRuntimeConfig(
                ...
                router_group_bias_scale=router_group_bias_scale,
    ...
    def forward(
        ...
        *,
    ...
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor], Dict[str, torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        if self.execution == "serial":
            out = hidden
            for stage_name in self.stage_order:
                out, w, l = self.branches[stage_name].forward_serial(
                    hidden=out,
                    item_seq=item_seq,
                    feat=feat,
            ...
            return out, gate_weights, gate_logits, None, None

        # parallel
        base_hidden = hidden
        stage_deltas: Dict[str, torch.Tensor] = {}
        for stage_name in self.stage_order:
            _stage_out, delta, w, l = self.branches[stage_name].forward_parallel(
        ...
        return merged_hidden, gate_weights, gate_logits, merge_w, merge_l
