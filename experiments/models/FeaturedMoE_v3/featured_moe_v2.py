...
class FeaturedMoE_V3(SequentialRecommender):
    """FeaturedMoE_v3: restored flat-router boundary-explicit Stage-MoE recommender."""
    ...
    def __init__(self, config, dataset):
        ...
        self.router_clone_residual_scale = float(resolver.get("router_clone_residual_scale", 0.5))
        ...
        self.router_distill_mode = str(resolver.get("router_distill_mode", "none")).lower().strip()
        self.router_distill_lambda_group = float(resolver.get("router_distill_lambda_group", 0.0))
        self.router_distill_lambda_clone = float(resolver.get("router_distill_lambda_clone", 0.0))
        ...
        valid_distill_modes = {"none", "group_only", "clone_only", "group_plus_clone"}
        if self.router_distill_mode not in valid_distill_modes:
            raise ValueError(
                f"router_distill_mode must be one of {sorted(valid_distill_modes)}, got {self.router_distill_mode}"
            )
        ...
        try:
            ...
            config["router_distill_mode"] = self.router_distill_mode
            config["router_distill_lambda_group"] = float(self.router_distill_lambda_group)
            config["router_distill_lambda_clone"] = float(self.router_distill_lambda_clone)
        ...
        logger.info(
            "FeaturedMoE_v3 init: layout_id=%s execution=%s global_pre=%s global_post=%s boundaries=%s total_moe_blocks=%s "
            ...
            "router_impl=%s router_impl_by_stage=%s router_design=%s distill_enable=%s distill_mode=%s "
            "distill_lambda_group=%s distill_lambda_clone=%s rule_router=%s",
            self.layout_id,
            ...
            self.router_distill_mode,
            self.router_distill_lambda_group,
            self.router_distill_lambda_clone,
    ...
    def calculate_loss(self, interaction):
        ...
        if self.use_aux_loss:
            ...
            aux_loss = aux_loss + compute_router_distill_aux_loss(
                gate_logits=aux_data.get("gate_logits", {}),
                feat=feat,
                stage_group_feature_indices=self.stage_group_feature_indices,
                item_seq_len=item_seq_len,
                expert_scale=self.expert_scale,
                mode=self.router_distill_mode,
                enabled=self.router_distill_enable,
                lambda_group=self.router_distill_lambda_group,
                lambda_clone=self.router_distill_lambda_clone,
