...
from ..FeaturedMoE.routers import RouterBackend, load_balance_loss
...
class MoEStage(nn.Module):
    ...
    def __init__(
        ...
        router_clone_residual_scale: float = 0.5,
    ...
    def _compute_learned_router(
        ...
        weights = _softmax_with_top_k(scaled_logits, n_experts=self.n_experts, top_k=self.current_top_k)

        if self.router_mode == "session":
            weights = self._expand_session_router_tensor(weights, seq_len=stage_feat.shape[1])
    ...
    def forward(
        ...
        if (
            ...
            router_stage_feat = router_stage_feat * rel
        ...
        if self.router_impl == "learned":
            if self.router_design == "flat_legacy":
                ...
                gate_weights, gate_logits = self._compute_learned_router(
                    h_norm=h_norm,
