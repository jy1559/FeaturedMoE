...
class MoEStageV2(nn.Module):
    ...
    def forward(
        ...
        return next_hidden, stage_delta, gate_w, gate_l
...
class StageBranchRunner(nn.Module):
    ...
    def forward_serial(
        ...
        *,
        ...
        for idx, pre_block in enumerate(self.moe_pre_blocks, start=1):
            ...
            out, _delta, w, l = self.stage_module(out, feat, item_seq_len=item_seq_len)
    ...
    def forward_parallel(
        ...
        *,
        ...
        logits: Dict[str, torch.Tensor] = {}
        if self.stage_module is None:
            return out, (out - base_hidden), weights, logits
