"""FeaturedMoE_HiR: Hierarchical-in-Stage Mixture-of-Experts recommender.

Compared to FeaturedMoE:
- Keep layout-based attention scaffold (global pre/post + stage pre blocks).
- Replace stage MoE with 2-level routing inside each stage:
  bundle router -> intra-bundle expert router.
- Experts are hidden-only FFNs (feature is used for routing only).
- Support stage merge mode:
  * serial   : Macro -> Mid -> Micro (default)
  * parallel : per-stage branch deltas merged by learned stage gate
"""
...
from .hir_moe_stages import HierarchicalMoEHiR
...
class FeaturedMoE_HiR(SequentialRecommender):
    """Layout-based Transformer + Hierarchical-in-Stage MoE."""
    ...
    def __init__(self, config, dataset):
        ...
        logger.warning(
            "[FeaturedMoE_HiR Layout] arch_layout_id=%d selected_layout=%s input_depth=%s order=%s",
            self.arch_layout_id,
        ...
        # Keep runtime config aligned for downstream logging.
        try:
        ...
        if not self.expert_use_hidden:
            raise ValueError("FeaturedMoE_HiR requires expert_use_hidden=True (experts are hidden-only).")
        if self.expert_use_feature:
            logger.warning(
                "FeaturedMoE_HiR uses hidden-only experts: expert_use_feature=True is ignored."
            )
        self.expert_use_feature = False

        # ---- MoE config ----
        raw_top_k = _cfg("moe_top_k", 0)
        self.moe_top_k = None if raw_top_k == 0 else int(raw_top_k)
        raw_bundle_top_k = int(_cfg("bundle_top_k", 0))
        self.bundle_top_k = None if raw_bundle_top_k <= 0 else raw_bundle_top_k
        ...
        self.hir_use_bundle_aux_loss = bool(_cfg("hir_use_bundle_aux_loss", True))
        self.hir_bundle_aux_lambda_scale = float(_cfg("hir_bundle_aux_lambda_scale", 1.0))
        ...
        # ---- Training schedules (optional; default OFF) ----
        ...
        # Stage-pre attention blocks (depth-many attention blocks before stage MoE)
        ...
        # Hierarchical-in-Stage MoE modules
        if self.any_moe:
            self.hierarchical_moe = HierarchicalMoEHiR(
                d_model=self.d_model,
                ...
                bundle_top_k=self.bundle_top_k,
                ...
                hir_use_bundle_aux_loss=self.hir_use_bundle_aux_loss,
                hir_bundle_aux_lambda_scale=self.hir_bundle_aux_lambda_scale,
            )
        else:
        ...
        logger.info(
            "FeaturedMoE_HiR: d_model=%s, d_feat_emb=%s, expert_scale=%s, "
            ...
            "heads=%s, active_stages=%s, merge_mode=%s, moe_top_k=%s, bundle_top_k=%s, "
            "parallel_stage_gate_top_k=%s, use_aux_loss=%s, bundle_aux=%s, bundle_aux_scale=%s, "
            "mid_temp=%s, micro_temp=%s, mid_feat_drop=%s, micro_feat_drop=%s, "
            "ffn_moe=%s, n_features=%s, fmoe_debug_logging=%s, "
            ...
            self.use_aux_loss,
            self.hir_use_bundle_aux_loss,
            self.hir_bundle_aux_lambda_scale,
    ...
    def _apply_training_schedule(self, log_now: bool = False) -> None:
        ...
        if should_log:
            logger.info(
                "FMoE_HiR schedule epoch=%s/%s alpha_scale=%.4f mid_temp=%.4f micro_temp=%.4f top_k=%s",
                epoch_idx + 1,
                total_epochs,
                alpha_scale,
                mid_temp,
    ...
    def forward(self, item_seq, item_seq_len, feat=None):
        ...
        gate_weights, gate_logits, bundle_weights = {}, {}, {}
            ...
            if self.stage_merge_mode == "parallel":
                ...
                for stage_name in _STAGE_NAMES:
                    ...
                    if stage_name in self.stage_pre_transformers:
                        stage_tokens, _ = self.stage_pre_transformers[stage_name](stage_tokens, item_seq)
                    ...
                    (
                        ...
                        _bl,
                        stage_delta,
                    ) = self.hierarchical_moe.forward_stage(
                        stage_name,
                        stage_tokens,
                        feat,
                    ...
                    bundle_weights[stage_name] = bw
                    stage_deltas[stage_name] = stage_delta

                tokens, stage_merge_weights, stage_merge_logits = self.hierarchical_moe.parallel_merge(
                    hidden=tokens,
                    feat=feat,
        ...
        if self.use_aux_loss and self.balance_loss_lambda > 0:
            if aux_data["gate_weights"] and self.hierarchical_moe is not None:
                aux_loss = self.hierarchical_moe.compute_aux_loss(
                    weights=aux_data["gate_weights"],
                    bundle_weights=aux_data.get("bundle_weights"),
                    item_seq_len=item_seq_len,
