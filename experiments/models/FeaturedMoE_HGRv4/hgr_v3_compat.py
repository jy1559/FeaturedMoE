"""Compat copy of HGRv3 base for active HGRv4 support."""
...
from .hgr_v3_compat_stages import HierarchicalMoEHGRv3
from .hgr_v3_compat_losses import (
    ...
    compute_inner_rule_distill_aux_loss,
)
...
class HGRv3StageBranchRunner(nn.Module):
    """Run one HGRv3 stage with optional pass layers and repeated MoE blocks."""
    ...
    def run_serial(
        ...
        *,
        ...
        hierarchical_moe: HierarchicalMoEHGRv3,
    ) -> Tuple[
        torch.Tensor,
        ...
        intra_group_logits_raw: Dict[str, torch.Tensor] = {}
        teacher_intra_group_logits: Dict[str, torch.Tensor] = {}
        inner_rule_score: Dict[str, torch.Tensor] = {}
        inner_rule_bin_centers: Dict[str, torch.Tensor] = {}
        ...
        if self.moe_blocks <= 0:
            return (
                ...
                intra_group_weights,
                intra_group_logits,
                intra_group_logits_raw,
                teacher_intra_group_logits,
                inner_rule_score,
                inner_rule_bin_centers,
            )
        ...
        for idx, pre_block in enumerate(self.moe_pre_blocks, start=1):
            ...
            if "intra_group_logits_raw" in router_aux:
                intra_group_logits_raw[key] = router_aux["intra_group_logits_raw"]
            if "teacher_intra_group_logits" in router_aux:
                teacher_intra_group_logits[key] = router_aux["teacher_intra_group_logits"]
            if "inner_rule_score" in router_aux:
                inner_rule_score[key] = router_aux["inner_rule_score"]
            if "inner_rule_bin_centers" in router_aux:
                inner_rule_bin_centers[key] = router_aux["inner_rule_bin_centers"]
        return (
            out,
            gate_weights,
            gate_logits,
...
class FeaturedMoE_HGRv3(SequentialRecommender):
    """Layout-based Transformer + hidden-only outer / inner-rule HGR."""
    ...
    def __init__(self, config, dataset):
        ...
        logger.warning(
            "[FeaturedMoE_HGRv3 Layout] arch_layout_id=%d selected_layout=%s "
        ...
        legacy_router_use_hidden = bool(_cfg("router_use_hidden", True))
        legacy_router_use_feature = bool(_cfg("router_use_feature", True))
        self.outer_router_use_hidden = bool(_cfg("outer_router_use_hidden", True))
        self.outer_router_use_feature = bool(_cfg("outer_router_use_feature", False))
        self.inner_router_use_hidden = bool(_cfg("inner_router_use_hidden", legacy_router_use_hidden))
        self.inner_router_use_feature = bool(_cfg("inner_router_use_feature", legacy_router_use_feature))
        ...
        if not (self.outer_router_use_hidden or self.outer_router_use_feature):
            raise ValueError("outer_router_use_hidden and outer_router_use_feature cannot both be false.")
        if not (self.inner_router_use_hidden or self.inner_router_use_feature):
            raise ValueError("inner_router_use_hidden and inner_router_use_feature cannot both be false.")
        ...
        if self.router_design != "group_factorized_interaction":
            raise ValueError("FeaturedMoE_HGRv3 supports only router_design=group_factorized_interaction")
        ...
        self.group_router_mode = "stage_wide"
        ...
        self.inner_rule_enable = bool(_cfg("inner_rule_enable", True))
        self.inner_rule_mode = str(_cfg("inner_rule_mode", "distill")).lower().strip()
        if self.inner_rule_mode not in {"off", "distill", "fused_bias", "distill_and_fused_bias"}:
            raise ValueError(
                "inner_rule_mode must be one of ['off','distill','fused_bias','distill_and_fused_bias']"
            )
        self.inner_rule_lambda = float(_cfg("inner_rule_lambda", 5e-3))
        self.inner_rule_temperature = float(_cfg("inner_rule_temperature", 1.5))
        self.inner_rule_until = float(_cfg("inner_rule_until", 0.2))
        self.inner_rule_bias_scale = float(_cfg("inner_rule_bias_scale", 1.0))
        self.inner_rule_bin_sharpness = float(_cfg("inner_rule_bin_sharpness", 16.0))
        self.inner_rule_group_feature_pool = str(_cfg("inner_rule_group_feature_pool", "mean_ratio")).lower().strip()
        self.inner_rule_apply_stages = self._parse_stage_list(
            _cfg("inner_rule_apply_stages", ["macro", "mid", "micro"]),
            default=["macro", "mid", "micro"],
        )
        self.inner_rule_distill_enable = bool(self.inner_rule_enable) and (
            self.inner_rule_mode in {"distill", "distill_and_fused_bias"}
        )
        ...
        if self.any_moe:
            self.hierarchical_moe = HierarchicalMoEHGRv3(
                d_model=self.d_model,
        ...
        try:
            ...
            config["inner_rule_enable"] = bool(self.inner_rule_enable)
            config["inner_rule_mode"] = str(self.inner_rule_mode)
            config["inner_rule_lambda"] = float(self.inner_rule_lambda)
            config["inner_rule_temperature"] = float(self.inner_rule_temperature)
            config["inner_rule_until"] = float(self.inner_rule_until)
            config["inner_rule_bias_scale"] = float(self.inner_rule_bias_scale)
            config["inner_rule_bin_sharpness"] = float(self.inner_rule_bin_sharpness)
            config["inner_rule_group_feature_pool"] = str(self.inner_rule_group_feature_pool)
            config["inner_rule_apply_stages"] = list(self.inner_rule_apply_stages)
        except Exception:
            pass
        ...
        logger.info(
            "FeaturedMoE_HGRv3: d_model=%s d_feat_emb=%s expert_scale=%s layout_id=%s layout=%s "
            ...
            "group_feature_spec_lambda=%s group_feature_spec_stages=%s inner_rule_enable=%s "
            "inner_rule_mode=%s inner_rule_lambda=%s inner_rule_temperature=%s inner_rule_until=%s "
            "inner_rule_bias_scale=%s inner_rule_apply_stages=%s "
            "parallel_stage_gate_top_k=%s "
            "parallel_stage_gate_temperature=%s macro_scope=%s macro_pool=%s "
            "outer_router_use_hidden=%s outer_router_use_feature=%s "
            "inner_router_use_hidden=%s inner_router_use_feature=%s "
            ...
            self.inner_rule_mode,
            self.inner_rule_lambda,
            self.inner_rule_temperature,
            self.inner_rule_until,
            self.inner_rule_bias_scale,
            self.inner_rule_apply_stages,
    ...
    def _reset_router_epoch_stats(self) -> None:
        ...
        self._router_teacher_clone_entropy_sum = defaultdict(float)
        self._router_teacher_clone_entropy_count = defaultdict(int)
    ...
    def _accumulate_router_stats(
        ...
        *,
        group_weights: Dict[str, torch.Tensor],
        intra_group_weights: Dict[str, torch.Tensor],
        teacher_intra_group_logits: Optional[Dict[str, torch.Tensor]],
        item_seq_len: Optional[torch.Tensor],
    ) -> None:
        ...
        for stage_key, teacher_logits in (teacher_intra_group_logits or {}).items():
            _, tlen, _, _ = teacher_logits.shape
            if item_seq_len is not None:
                lens = item_seq_len.to(device=teacher_logits.device).long()
                valid = torch.arange(tlen, device=teacher_logits.device).unsqueeze(0) < lens.unsqueeze(1)
                flat_teacher = teacher_logits[valid]
            else:
                flat_teacher = teacher_logits.reshape(-1, teacher_logits.shape[-2], teacher_logits.shape[-1])
            if flat_teacher.numel() == 0:
                continue
            teacher_prob = torch.softmax(flat_teacher, dim=-1)
            entropy = -(teacher_prob.clamp(min=1e-8) * teacher_prob.clamp(min=1e-8).log()).sum(dim=-1)
            self._router_teacher_clone_entropy_sum[stage_key] += float(entropy.sum().item())
            self._router_teacher_clone_entropy_count[stage_key] += int(entropy.numel())
    ...
    def forward(self, item_seq, item_seq_len, feat=None):
        ...
        intra_group_weights, intra_group_logits, intra_group_logits_raw = {}, {}, {}
        teacher_intra_group_logits, inner_rule_score, inner_rule_bin_centers = {}, {}, {}
        stage_weights = None
        stage_logits = None
        ...
        if self.any_moe and feat is not None and self.hierarchical_moe is not None:
            if self.stage_merge_mode == "parallel":
                ...
                for stage_name in _STAGE_NAMES:
                    ...
                    _, stage_delta, w, l, gw, gl, igw, igl, iglr, tigl, irs, irc = self.stage_branches[stage_name].run_parallel(
                        base_hidden=tokens,
                        item_seq=item_seq,
                        feat=feat,
                        item_seq_len=item_seq_len,
                    ...
                    intra_group_logits_raw.update(iglr)
                    teacher_intra_group_logits.update(tigl)
                    inner_rule_score.update(irs)
                    inner_rule_bin_centers.update(irc)
                ...
                for stage_name in _STAGE_NAMES:
                    ...
                    tokens, w, l, gw, gl, igw, igl, iglr, tigl, irs, irc = self.stage_branches[stage_name].run_serial(
                        hidden=tokens,
                        item_seq=item_seq,
                        feat=feat,
                        item_seq_len=item_seq_len,
        ...
        aux_data = {
            ...
            "intra_group_logits_raw": intra_group_logits_raw,
            "teacher_intra_group_logits": teacher_intra_group_logits,
            "inner_rule_score": inner_rule_score,
            "inner_rule_bin_centers": inner_rule_bin_centers,
    ...
    def calculate_loss(self, interaction):
        ...
        if self.use_aux_loss:
            ...
            aux_loss = aux_loss + compute_inner_rule_distill_aux_loss(
                teacher_intra_group_logits=aux_data.get("teacher_intra_group_logits", {}),
                student_intra_group_logits_raw=aux_data.get("intra_group_logits_raw", {}),
                item_seq_len=item_seq_len,
                aux_lambda=self.inner_rule_lambda,
                distill_temperature=self.inner_rule_temperature,
                enabled=self.inner_rule_distill_enable,
                progress=float(self._schedule_epoch) / float(max(self._schedule_total_epochs - 1, 1)),
                until=self.inner_rule_until,
                device=ce_loss.device,
            )
    ...
    def get_epoch_log_summary(self) -> Dict:
        ...
        for stage_key in sorted(all_stage_keys):
            ...
            summary["router"][stage_key] = {
                ...
                "teacher_guided_clone_entropy": (
                    float(self._router_teacher_clone_entropy_sum.get(stage_key, 0.0))
                    / float(max(self._router_teacher_clone_entropy_count.get(stage_key, 0), 1))
                ),
            }
